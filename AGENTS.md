...
- 自托管 Runner：Plan 269 于 2025-11-22 正式取消 WSL Runner 例外，现阶段 **禁止启用 `self-hosted,cubecastle,wsl` 标签**，所有 workflow（含 `ci-selfhosted-smoke`）一律改在 GitHub 平台 runner (`ubuntu-latest`) 上运行。如确需自托管 Runner，必须另行立项并获批准；在新的审批落地前，不得执行 `scripts/ci/runner/*` 中的安装/启用脚本。
...
- 未上线彻底改造：本项目尚未正式对外提供服务，可在确保唯一事实来源的前提下进行彻底重构，不需为兼容旧路径或“正在运行的服务”保留折中方案；除非另有书面约束，默认直接替换旧实现而不是堆叠临时开关。
...
- PR 合并后的同步（强制，确保不丢失他人变更且不新增分支）：开发工作树保持在 `feat/shared-dev`，不切换；仅在只读工作树 `../cube-castle-master` 内执行快进
  ```bash
  git fetch --prune
  git pull --ff-only
  ```
...
- 授权范围（遵循上方约束与 Docker 强制原则）：
  ...
  - Docker：允许执行 `docker compose -f docker-compose*.yml up/down/build/pull/logs/ps/exec`；严禁修改端口映射；如遇 5432/6379 冲突，按“卸载宿主服务”原则处理。
  - 数据库：仅限容器内 PostgreSQL/Redis；允许迁移（`make db-migrate-all`）、只读排障（`psql`/`redis-cli`）与明确说明目的的最小必要写入（有回滚方案）。
- 审批策略：本地会话按“最小必要 + 可追溯”执行；高危操作仍需在对话中显式声明命令、影响面与回滚方案后方可实施。
