...
- 自托管 Runner 例外（Plan 269 批准，2025-11-20）：CI Runner 属于开发辅助设施，可在 WSL 内原生部署，但必须满足：
  - 业务服务依旧在 Docker 容器内启动，Runner 只负责调度作业；
  - WSL Runner 必须按 `docs/reference/wsl-runner-setup.md` 安装，具备 Go 1.24.9+/Node 18+/Docker CLI，且与 Docker Runner 保持同等 `runs-on` 标签（`self-hosted,cubecastle,wsl`）；
  - Docker Runner 作为回退路径长期保留，一旦 WSL Runner 不健康需立即切回；
