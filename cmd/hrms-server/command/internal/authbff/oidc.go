...
import (
    ...
    auth "cube-castle/cmd/hrms-server/command/internal/auth"
...
func (c *OIDCClient) ValidateIDToken(idToken string, expectedNonce string) (map[string]any, error) {
    ...
    keyFunc := func(token *jwt.Token) (interface{}, error) {
        ...
        if k := c.jwks.GetKey(kid); k != nil { return k, nil }
    ...
    if err != nil { return nil, fmt.Errorf("ID_TOKEN_INVALID: %w", err) }
    mc, ok := tok.Claims.(jwt.MapClaims)
    if !ok || !tok.Valid { return nil, fmt.Errorf("ID_TOKEN_INVALID_CLAIMS") }
    ...
    if audStr, ok := mc["aud"].(string); ok {
        ...
        for _, v := range audArr { if s, ok := v.(string); ok && s == c.cfg.ClientID { audOK = true; break } }
    }
    if !audOK { return nil, fmt.Errorf("ID_TOKEN_AUDIENCE_MISMATCH") }
    // exp/nbf 时间校验（含5分钟容忍）
    now := time.Now()
    if exp, ok := mc["exp"].(float64); ok {
        if now.After(time.Unix(int64(exp), 0).Add(5 * time.Minute)) { return nil, fmt.Errorf("ID_TOKEN_EXPIRED") }
    }
    if nbf, ok := mc["nbf"].(float64); ok {
        if now.Add(5 * time.Minute).Before(time.Unix(int64(nbf), 0)) { return nil, fmt.Errorf("ID_TOKEN_NOT_YET_VALID") }
    }
    // nonce 校验（如存在）
    if expectedNonce != "" {
...
func parseUnverifiedClaims(idToken string) (map[string]any, error) {
    ...
    if len(parts) != 3 { return nil, fmt.Errorf("invalid id_token format") }
