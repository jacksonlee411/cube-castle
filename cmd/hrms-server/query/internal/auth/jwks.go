...
func (m *JWKSManager) Refresh() error {
    ...
    if m.url == "" { return fmt.Errorf("jwks url empty") }
    ...
    for _, k := range set.Keys {
        ...
        if pk, err := RSAFromModExp(k.N, k.E); err == nil { nc[k.Kid] = pk }
    }
    if len(nc) > 0 { m.cache = nc; m.lastFetch = time.Now() }
    return nil
}

func ParseRSAPublicKeyFromPEM(pemBytes []byte) (*rsa.PublicKey, error) {
    block, _ := pem.Decode(pemBytes); if block == nil { return nil, fmt.Errorf("invalid pem") }
    pub, err := x509.ParsePKIXPublicKey(block.Bytes); if err != nil { return nil, err }
    rsaPub, ok := pub.(*rsa.PublicKey); if !ok { return nil, fmt.Errorf("not rsa public key") }
    return rsaPub, nil
}

func RSAFromModExp(nB64URL, eB64URL string) (*rsa.PublicKey, error) {
    nBytes, err := base64.RawURLEncoding.DecodeString(nB64URL); if err != nil { return nil, err }
    eBytes, err := base64.RawURLEncoding.DecodeString(eB64URL); if err != nil { return nil, err }
    var eInt int
