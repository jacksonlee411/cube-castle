-- Fix employee IDs and create position history
-- 修复员工ID并创建职位历史记录
...
-- 1. 创建UUID到员工ID的映射表并更新employees表
CREATE TEMPORARY TABLE emp_id_mapping AS
WITH new_uuids AS (
  SELECT 
    id as old_id,
    name,
...
SELECT * FROM new_uuids;

-- 备份旧数据并更新为UUID
UPDATE employees 
SET id = (SELECT new_id FROM emp_id_mapping WHERE old_id = employees.id)
WHERE id LIKE 'emp_%';

-- 2. 创建职位历史记录，使用真实的UUID
  ...
  SELECT 
    ...
    CASE 
      ...
      WHEN ph.job_level = 'EXECUTIVE' AND ph.position_title NOT LIKE '%CTO%' THEN 
        (SELECT employee_id FROM position_history WHERE position_title LIKE '%CTO%' AND tenant_id = :'tenant_id' LIMIT 1)
      WHEN ph.job_level = 'DIRECTOR' THEN
        CASE 
          WHEN ph.department IN ('前端开发部', '后端开发部', '移动开发部', '数据工程部', 'DevOps部', '测试部') THEN
            (SELECT employee_id FROM position_history WHERE position_title LIKE '%VP Engineering%' AND tenant_id = :'tenant_id' LIMIT 1)
          WHEN ph.department = '产品管理部' THEN
            (SELECT employee_id FROM position_history WHERE position_title LIKE '%CPO%' AND tenant_id = :'tenant_id' LIMIT 1)
          WHEN ph.department = '销售部' THEN
            (SELECT employee_id FROM position_history WHERE position_title LIKE '%VP Sales%' AND tenant_id = :'tenant_id' LIMIT 1)
          WHEN ph.department IN ('财务部', '人力资源部') THEN
            (SELECT employee_id FROM position_history WHERE position_title LIKE '%CFO%' AND tenant_id = :'tenant_id' LIMIT 1)
          ELSE (SELECT employee_id FROM position_history WHERE position_title LIKE '%CTO%' AND tenant_id = :'tenant_id' LIMIT 1)
        END
      WHEN ph.job_level IN ('PRINCIPAL', 'SENIOR', 'MANAGER') THEN
        (SELECT employee_id FROM position_history WHERE job_level = 'DIRECTOR' AND department = ph.department AND tenant_id = :'tenant_id' LIMIT 1)
      WHEN ph.job_level IN ('REGULAR', 'JUNIOR', 'INTERN') THEN
        COALESCE(
          (SELECT employee_id FROM position_history WHERE job_level IN ('SENIOR', 'MANAGER') AND department = ph.department AND tenant_id = :'tenant_id' LIMIT 1),
          (SELECT employee_id FROM position_history WHERE job_level = 'DIRECTOR' AND department = ph.department AND tenant_id = :'tenant_id' LIMIT 1)
        )
      ELSE NULL
    END as manager_id
  FROM position_history ph
  WHERE ph.tenant_id = :'tenant_id' AND ph.end_date IS NULL
)
...
WHERE position_history.employee_id = rs.employee_id 
  AND position_history.tenant_id = :'tenant_id'
...
SELECT COUNT(*) as current_positions FROM position_history WHERE tenant_id = :'tenant_id' AND end_date IS NULL;
...
-- 清理临时表
DROP TABLE IF EXISTS emp_id_mapping;