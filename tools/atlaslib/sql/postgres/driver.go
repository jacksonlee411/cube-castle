...
func Open(db schema.ExecQuerier) (migrate.Driver, error) {
	...
	if rows, err := db.QueryContext(context.Background(),
		...
		var collate, ctype sql.NullString
		if err := sqlx.ScanOne(rows, &collate, &ctype); err != nil && !errors.Is(err, sql.ErrNoRows) {
			return nil, fmt.Errorf("postgres: scanning database collation: %w", err)
		}
		if sqlx.ValidString(collate) {
			c.collate = collate.String
		}
		if sqlx.ValidString(ctype) {
			c.ctype = ctype.String
		}
	} else if !errors.Is(err, sql.ErrNoRows) {
		return nil, fmt.Errorf("postgres: querying database collation: %w", err)
	}
