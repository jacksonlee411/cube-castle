...
- **代码基线**：
  ...
  - Go 版本与工具链与 `go.mod` 对齐（Plan 204 默认 go1.22.x，如需调整应先提交 PLAN200 系列变更；当前执行阶段因 `github.com/jackc/pgx/v5` 等依赖已暂定 `go1.24.0`，待 Steering 确认后更新基线）。
...
| 角色 | 主要职责 | 交付物 |
|------|----------|--------|
| 全栈执行负责人（Codex） | 统筹 Day1-10 执行、协调 QA/DevOps/文档、维护事实来源 | Plan 211 执行日志、`reports/phase1-module-unification.md`、回归记录 |
| 架构师 | 定义模块命名与目录蓝图；评估结构调整风险；把关资源唯一性 | Kick-off 纪要、结构审查记录 |
| 后端 TL | 统筹迁移分支、拆分任务、审阅关键 PR；形成 go.mod 合并策略 | go.mod 合并策略、迁移 PR 审阅记录 |
| 后端开发 | 执行代码迁移、路径调整、共享代码抽取；修复编译/测试问题 | 更新后的代码、变更说明、临时脚本（含 `// TODO-TEMPORARY:`） |
| QA | 制定并执行回归计划（单测/集成）；记录测试结果 | 回归报告 `reports/phase1-regression.md` |
| DevOps | 更新 Makefile/Docker/CI 工作流；完成测试环境部署与监控 | 配置变更说明、部署日志、环境基线文件 |
...
| Day2 | 1.1 模块命名 | 审核并锁定 `module cube-castle`；若需路径前缀调整提交方案 | 架构师 | `docs/development-plans/211-Day2-Module-Naming-Record.md` |
| Day2 | 分支准备 | 建立 `feature/204-phase1-unify` 分支，配置 CI | 后端 TL | 分支策略说明 |
| Day3 | 1.2 go.mod 差距盘点 | 列出 5 个现有 go.mod 与 go.work 使用情况，形成合并策略草案 | 后端 TL | go.mod 合并策略（草案） |
| Day3 | 依赖审计准备 | 识别私有依赖、replace 规则、工具链版本差异 | 架构师 | 依赖审计初稿 |
| Day4 | 1.2 go.mod 合并执行 | 按策略合并根 `go.mod`，移除冗余 go.mod/go.work，确保 `go list ./...` 通过 | 后端 TL | 更新后的 go.mod/go.sum |
| Day4 | 构建配置准备 | 梳理 Makefile、Docker、CI 需要调整的路径与变量 | DevOps | 配置调整清单 |
| Day5 | 1.3/1.4 迁移执行 | 并行迁移 command、query 入口并修复引用 | 后端开发（2人） | 迁移 PR（command/query） |
| Day5 | 构建与 CI 清理 | 根据清单更新 Makefile、Docker、CI 工作流并移除 Neo4j 依赖 | DevOps | 配置变更说明 |
| Day6-7 | 1.5 共享代码抽取 | 分类整理 `/internal` 与 `/pkg`；确保 CQRS 边界；同步 README | 后端团队 + 文档 | 分类清单、文档更新 |
| Day6-7 | 架构审查 | 检查抽取是否破坏模块边界，必要时回滚问题提交 | 架构师 | 审查意见 |
| Day8 | 1.6 编译与测试 | 执行 `go test ./...`、`make test`、`npm run lint`；修复失败并补充数据一致性验证脚本 | QA + 后端 | 测试记录 |
| Day9 | 1.7 部署测试环境 | 重新构建镜像，部署至测试环境，更新基线日志 | DevOps | 部署日志、`baseline-*.log` |
| Day9 | 健康检查 | `curl` 健康端点，确认服务就绪 | QA | 健康检查记录 |
| Day10 | 1.8 回归总结 | 汇总功能回归结果、数据一致性校验与遗留问题 | QA | `reports/phase1-regression.md` |
