...
## 0. 当前执行状态（2025-11-19）
- ✅ Runner 已全面 Compose 化：`docker-compose.runner.persist.yml` 使用自建镜像 `cc-actions-runner-docker:2.329.0`（内置 docker-compose-plugin、公共 DNS），并通过 `runner/persistent-entrypoint.sh` 幂等配置，配合 `scripts/ci/runner/start-ghcr-runner-persistent.sh --build`、`watchdog.sh` 保活。
- ✅ 自托管冒烟与诊断均完成一次成功 run：  
  - `CI (Self-Hosted Runner Smoke)` run 19485705844（2025-11-19 01:21 UTC）成功拉起 postgres/redis、跑通健康检查。  
  - `CI (Self-Hosted Runner Diagnose)` run 19486773039（2025-11-19 01:43 UTC）确认 runner 内 Docker/Compose 正常，可拉起临时容器。
- ✅ 网络与健康检查策略已固化：通过 compose 项目隔离 + `docker inspect` 自定义轮询，避免 `docker compose wait` 与 `ps --format json` 在 Runner 容器中的兼容性问题。
- ⚠️ 下一阶段仍需执行 T+1/T+2 任务：为重型 workflow（前端质量门禁、E2E 等）增加 self-hosted 矩阵，并观察长时间运行下 runner 的稳定性及磁盘占用。
...
## 2. 方案概述
- 架构（已实施：方案A Persistent）：基于自建镜像 `cc-actions-runner-docker:2.329.0`（官方 `ghcr.io/actions/actions-runner` 扩展，预装 docker compose 插件 + 公共 DNS）启动“持久化 Runner”，通过 `scripts/ci/runner/start-ghcr-runner-persistent.sh` 注册并常驻，结合 `scripts/ci/runner/watchdog.sh` 每 60s 健康巡检，异常自动重拉。
...
1) 准备密钥（任一）
   ...
   ```
2) 启动 Runner（容器化，方案A）
   - 通过 `scripts/ci/runner/start-ghcr-runner-persistent.sh` 获取注册令牌并执行 `docker compose -f docker-compose.runner.persist.yml up -d --build`，自动构建/拉取自定义镜像、注册 runner 并保持在线。
   - `docker-compose.runner.persist.yml` 入口会检查 `.runner/.credentials*` 后再执行 `config.sh`，避免 “already configured”；若需要重配可设置 `FORCE_RECONFIGURE=true`。
...
4) 验证工作流（示例）
   - 手动触发 `.github/workflows/ci-selfhosted-diagnose.yml`；应在自托管 Runner 执行并通过基本检查。最近一次成功 run：workflow_dispatch #19486773039（2025-11-19 01:43 UTC），日志已存档于 `logs/ci-monitor/run-19486773039`.
   - 手动触发 `.github/workflows/ci-selfhosted-smoke.yml`；最近一次成功 run：push #19485705844（2025-11-19 01:21 UTC），可作为 Compose 工作负载验收基线。
...
- T+0/1：本地或 CI 主机启动 Runner，验证 smoke/diagnose —— 已完成（run 19485705844、19486773039）；后续持续观察 runner 稳定性。
- T+1/2：选择 1~2 个重型工作流做 matrix 试跑；**行动项**：优先挑选 `.github/workflows/fronted-quality-gate.yml`、`e2e-tests.yml` 等重型流水线添加自托管矩阵。
- 产物：`docker-compose.runner.yml`、`docker-compose.runner.persist.yml`、`.github/workflows/ci-selfhosted-smoke.yml`、`.github/workflows/ci-selfhosted-diagnose.yml`、`scripts/ci/runner/README.md`。
