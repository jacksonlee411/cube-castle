...
### Phase 1 — 根因定位（1 天）
1. ✅ 2025-10-07 跟踪查询服务链路：确认 GraphQL Resolver → Repository 无额外过滤，`processAuditRowsStrict` 仅剔除空变更记录，未导致 500/空数组。
2. ✅ 2025-10-07 对照命令服务触发器与近期迁移：定位 `database/migrations/031_cleanup_temporal_triggers.sql` 版本的 `log_audit_changes()` 移除字段差异计算，导致 `changes`/`dataType` 丢失。
3. ✅ 2025-10-07 根因记录已补充至本文第 5 节与 `docs/development-plans/06-integrated-teams-progress-log.md`（待同步阶段见下文）。
...
1. 根据根因选择修复策略（示例）：
   ...
   - 数据层：通过迁移或脚本补齐历史数据。
     ...
     - ✅ 2025-10-07 新增并执行 `database/migrations/034_rebuild_audit_trigger_with_diff.sql`，重建 `log_audit_changes()`，恢复字段差异与 `dataType` 写入。
     - ✅ 2025-10-07 使用 `jsonb_set` 补齐历史审计记录 `5a380d66-e581-4700-b7f3-803042babd7c` 的缺失 `dataType`。
2. 补充单元测试 / 集成测试：
   - Go：`cmd/organization-query-service/internal/.../_test.go`
   - GraphQL：新增契约测试验证非空约束。
     - ✅ 2025-10-07 查询服务重启后通过 `curl` 调用 GraphQL `auditHistory`，返回 3 条记录且 `changes[].dataType` 均符合契约（详见 `reports/temporal/audit-history-nullability-20251007.log` 与同名报告）。
3. 执行验证命令：
   - `make test`
   - `make test-integration`
   - `npm --prefix frontend run test:e2e -- --grep "audit"`（如剧本存在）
   - ✅ `go test ./cmd/organization-query-service/internal/repository`（2025-10-07）验证触发器重建后仓储解析仍通过单测。
   - ✅ `make db-migrate-all`（2025-10-07）确保所有迁移落地，并通过 `psql` 手动触发两次 UPDATE 验证新触发器输出。
