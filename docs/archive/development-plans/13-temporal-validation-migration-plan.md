...
- `frontend/src/shared/validation/schemas.ts:189` 已声明替代路径，`frontend/src/shared/utils/index.ts` 已在 v2.0.0 统一导出 `TemporalConverter/TemporalUtils`，现有遗留引用全部集中在 feature 层组件内。
...
| 序号 | 验收项 | 说明 |
|------|--------|------|
| G1 | 所有前端引用切换至 `shared/utils/temporal-validation-adapter.ts` / `shared/utils/temporal-converter.ts` 或 `shared/validation/schemas.ts` | 通过脚本 + 手动审查验证 `rg 'temporalValidation'` 为空 |
...
2. **适配层封装**：在 `shared/utils/` 下新增 `temporal-validation-adapter.ts`，提供与旧 `validateTemporalDate` 等价的包装函数（保持调用签名），内部调用 `TemporalConverter/TemporalUtils`。
3. **自动迁移脚本**：在 `frontend/scripts/migrations/` 下新增 `20250921-replace-temporal-validation.ts`，使用 TypeScript AST（ts-morph）批量重写 import 指向新适配层。
4. **手动兜底**：对脚本未覆盖的业务自定义逻辑进行代码评审与微调，确保逻辑等价。
5. **Schema 委托**：同步更新 `shared/validation/schemas.ts`，将时态相关的 Zod 校验逻辑委托给 `TemporalUtils`，删除重复工具函数实现，保持单一事实来源。
6. **验证 & 清理**：执行前端测试、快照更新，最终删除临时文件并更新 TODO 清单。
...
- [ ] 负责人：Jinwei（前端架构组）
  ...
  - 任务 A2：确认 `temporalValidation.ts` 每个方法在新工具中一一对应，列出所需包装逻辑（如 `isFutureDate` 的零点比较、`isInRange` 的缺省边界处理）。
  - 任务 A3：在 `TemporalUtils` 中补齐缺失的功能或守卫（如允许缺省边界的 `isInRange` 兼容逻辑），并同步更新单元测试，确保能力覆盖。
  - 任务 A4：新增 `shared/utils/temporal-validation-adapter.ts`（命名待确认），导出 `validateTemporalDate` 对象与旧接口保持完全一致的调用签名，同时补充单元测试验证包装行为。
  - 产出：方法映射表、潜在差异点列表。
  - 备注：所有 Phase A 差异修复必须在脚本开发启动前合入，以免自动化迁移过程中触发逻辑回归。
...
- [ ] 负责人：Yuxi（时态体验小组）
  - 任务 B1：在 `frontend/scripts/` 下创建 `migrations/` 子目录，并新增 `20250921-replace-temporal-validation.ts`，支持以下能力：
    ...
    2. 自动替换 `import { validateTemporalDate } from '../utils/temporalValidation';` 为 `import { validateTemporalDate } from '@/shared/utils/temporal-validation-adapter';`（含绝对路径别名处理）。
    3. 对命名空间调用（例如 `validateTemporalDate.isFutureDate`）保持不变，只调整导入来源，确保调用签名无需重写。
    4. 检测 `validateTemporalDate` 在解构或别名场景下的使用，依次替换为新适配层导出，确保 ESLint 通过。
  ...
  - 任务 B3：脚本执行后自动移除不再需要的相对路径导入，保持 ESLint 通过。
  - 任务 B4：更新 `frontend/scripts/README.md` 文档，记录脚本使用方式、依赖安装步骤与回滚说明。
  - 任务 B5：在提交主干前对小范围文件（如 `TemporalDatePicker`、`TemporalMasterDetailView`）执行脚本试跑（含 `--check` 与真实执行），记录差异并反馈至 Phase A/B 清单，确认自动化策略可安全推广。

### Phase C — 执行迁移（1 天）
- [ ] 负责人：Jinwei
  - 任务 C1：在开发分支运行 `npm run migrate:temporal-validation -- --check`，确认输出与预期一致后执行正式迁移。
  - 任务 C2：对以下关键文件进行手动审查，确保逻辑等价（重点确认适配层包装后仍满足业务约束）：
  ...
  - 任务 C4：重构 `shared/validation/schemas.ts`，让时态相关 Schema 直接复用 `TemporalUtils` / `TemporalConverter` 能力（例如 `temporal` 子对象调用统一工具而非自实现），并验证删除重复逻辑后测试仍通过。
...
| 风险 | 描述 | 缓解措施 |
|------|------|----------|
| R1 | 脚本替换遗漏动态引用或别名导出 | 结合 `git diff` + 手动代码审查；保留脚本 dry-run 模式；对脚本识别不到的动态 import 做人工补漏 |
...
## 9. 输出与交付物
1. `frontend/src/shared/utils/temporal-validation-adapter.ts`（命名待确认）与配套单元测试
2. `frontend/scripts/migrations/20250921-replace-temporal-validation.ts`
3. 更新后的前端源码（所有引用指向新适配层）
4. 删除临时文件与 TODO 记录
5. 验证报告：测试日志、关键页面截图
6. 文档与报告更新：实现清单（Markdown + JSON）、未决问题列表、IIG 报告
