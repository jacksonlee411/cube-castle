...
## 13) 数据巡检结论（POSITION/JOB_CATALOG 范围评估）

- 结论：本环境中 POSITION/JOB_CATALOG 均不存在历史 `ACTIVATE` 事件（同时无 `REACTIVATE/SUSPEND/DEACTIVATE`）；组织域亦未发现 `ACTIVATE` 事件。
- 当前数据分布（仅列关键项）：
  - ORGANIZATION: CREATE=848, UPDATE=2, ERROR=22
  - JOB_CATALOG: CREATE=2, UPDATE=2
  - POSITION: ERROR=6
- 决策：迁移范围无需扩大到 POSITION/JOB_CATALOG；组织域的“ACTIVATE→REACTIVATE”迁移保留为“幂等兜底项”，在其它环境如检测到 `ACTIVATE` 再执行。
- 复现命令（容器内只读）：
  - 汇总统计：
    - `docker compose -f docker-compose.dev.yml exec -T postgres psql -U user -d cubecastle -c "SELECT resource_type, event_type, count(*) FROM audit_logs GROUP BY 1,2 ORDER BY 1,2;"`
  - 巡检脚本（含采样与质量项）：
    - `docker compose -f docker-compose.dev.yml exec -T postgres psql -U user -d cubecastle -v ON_ERROR_STOP=1 -f - < sql/inspection/audit-activate-coverage.sql`
- 产物：`sql/inspection/audit-activate-coverage.sql`（只读巡检脚本，便于在其它环境快速评估是否需要执行迁移）。
