...
- 临时方案治理：临时兼容以 `// TODO-TEMPORARY(YYYY-MM-DD):` 标注并登记，回收期不超过一个迭代。  
...
3. 232/232T 的 P0 用例持续失败与等待链路超时（docs/archive/development-plans/232t-test-checklist.md:25-69），说明前端数据加载与可观测性必须统一治理，而非在单个页面内“补丁式”处理。
