...
**创建日期**: 2025-09-08  
**维护团队**: 项目维护团队  
**遵循原则**: CLAUDE.md 资源唯一性和命名规范原则（最高优先级：统一事实来源与跨文档一致性）

## 🎯 文档管理目标

基于CLAUDE.md第10条"资源唯一性和命名规范原则"，建立统一的文档管理标准，防止文档散乱、重复和过时问题的再次出现；任何破坏唯一事实来源或文档一致性的行为被视为最高优先级阻断项。
...
### 1. 新文档创建规范
- **最高优先级检查**: 创建前必须验证不会引入第二事实来源或跨层不一致，否则不得创建。
...
- **一致性检查（最高优先级）**: 更新时需要检查相关文档的一致性并记录校验方法，缺失即禁止合并
...
### 绝对禁止
- **破坏唯一事实来源**: 引入重复的权威文档或分叉事实，必须立即回滚。
