...
### ✨ 新增 - 时态版本管理API端点
- **新增REST端点**: `POST /api/v1/organization-units/{code}/versions` - 为现有组织创建新的时态版本
  - **功能**: 支持历史数据补正和未来生效版本创建
  - **自动化**: 自动管理is_current状态转换和end_date计算
  - **验证**: 完整的日期冲突检查和业务规则验证

### 📋 Schema定义扩展
- **新增Schema**: `CreateVersionRequest` - 时态版本创建请求结构
  - **字段**: name, unitType, parentCode, description, effectiveDate, endDate, operationReason
  - **验证**: 日期格式校验、字段长度限制、业务规则检查
- **响应增强**: 支持详细的冲突信息和版本状态返回

### 🔧 解决核心问题
- **修复**: 前端插入新版本功能缺失对应API端点的问题
- **改进**: 提供符合CQRS架构的标准化时态数据管理接口
- **增强**: 支持数据库现有的26个时态专用索引和自动化触发器

### 📈 版本升级指南
- **前端**: 将错误的`/events`端点调用替换为新的`/versions`端点
- **请求结构**: 使用`CreateVersionRequest`格式替代原有`events`格式
- **错误处理**: 新增409冲突响应和详细错误信息处理
