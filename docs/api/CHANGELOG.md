...
## [v4.6.2] - 2025-09-15 🔒 审计触发器退场与应用层审计统一

### 变更 - 审计产出机制统一（迁移说明）
- 移除数据库行级审计触发器（log_audit_changes/audit_changes_trigger），由应用层结构化审计作为单一事实来源。
  - 数据库迁移：
    - 027_fix_audit_trigger_op_type_from_tg_op.sql（过渡修复，避免读取已移除列 NEW.operation_type）
    - 028_drop_legacy_row_audit_trigger.sql（最终移除行级触发器与函数）
  - 理由：
...
- 监控统计改为派生表达式：
  - 未来记录：`effective_date > CURRENT_DATE`
  - 历史记录：`end_date IS NOT NULL AND end_date <= CURRENT_DATE`
  - 不再引用已移除列 `is_future`。

### 兼容性
- API 无破坏性变更；数据库层仅移除了内部用触发器，不影响外部契约。
- 应用层审计为权威机制，客户端查询/展示不受影响。

### 升级步骤（运行时）
1) 按序执行数据库迁移 `database/migrations/027_*.sql` 与 `028_*.sql`。
2) 重启命令服务，使监控与审计写入逻辑生效。
