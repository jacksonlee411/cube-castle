...
组织单元管理API提供完整的企业组织架构管理功能，基于CQRS架构实现高性能读写分离，支持双路径API访问(标准格式+CoreHR格式)，层级结构、多种单元类型和灵活的配置选项，实现多租户隔离和完整的CRUD操作。

### 🚀 CQRS架构特性 ⭐

**双路径API支持**:
- **标准路径**: `/api/v1/organization-units` (现有格式，向后兼容)
- **CoreHR路径**: `/api/v1/corehr/organizations` (企业级HR格式)

**技术架构**:
- **查询端**: Neo4j图数据库，优化层级查询性能
- **命令端**: PostgreSQL事务存储，保证数据一致性
- **事件驱动**: Kafka实时同步，保证最终一致性
...
**统一API网关**: `http://localhost:8000`

**支持的API路径**:
- **标准路径**: `/api/v1/organization-units` (现有格式，完全向后兼容)
- **CoreHR路径**: `/api/v1/corehr/organizations` (企业级HR标准格式)
  ...
  "organizations": [
    {
      ...
      "updated_at": "2025-08-06T06:13:47.072807Z"
    }
  ],
  "total_count": 8,
  "page": 1,
