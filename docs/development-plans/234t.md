...
- [x] Schema、Goose 迁移、断言脚本与文档均已提交，触发器逻辑在代码层面完成移除。
- [ ] 由于 CLI 环境无法访问 Docker daemon 与 `localhost:5432`，未能在容器网络中执行 `make db-migrate-all` 及两份验证脚本；`logs/Plan234/*` 仍为空。
- [ ] 上述验证需在具备 Docker 权限的环境中按“验证步骤”执行，并将真实输出与日志路径补回本文档，方可宣布 Plan 234T 验证通过。
