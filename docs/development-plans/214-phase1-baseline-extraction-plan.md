...
| 日期 | 任务 | 负责人 | 输出 |
|------|------|--------|------|
| Day1 上午 | 冻结/确认当前数据库状态（依赖 210 Phase0 已完成） | 基础设施组 | 状态记录 |
| Day1 下午 | 使用 Docker Postgres 生成 `current_schema.sql`；atlas schema inspect 生成 diff | DBA | D1 |
| Day2 上午 | 整理 `database/schema.sql`（按 203/205 约定命名、约束） | 架构组 | D2 draft |
| Day2 下午 | 通过 Atlas 或手工生成基线迁移；补全 Down 脚本 | DBA + 架构组 | D3 draft |
| Day3 上午 | Up/Down 本地回放验证（`goose up/down` + `go test ./...`） | DBA | 验证日志 |
| Day3 下午 | 架构组/DBA 联合评审，收集意见并修订 | 架构组 + DBA | D3 最终版 |
| Day4 上午 | 生成签字纪要，归档日志、脚本；更新 210/06/Plan 214 文档状态 | PM + Codex | D4, D5 |
...
- 迁移文件命名：`database/migrations/20251105000000_base_schema.sql`，需包含：
  ```sql
  -- +goose Up
  ...
  -- +goose Down
...
### 4.5 审阅与签字
- 通过 PR 或线下会议收集意见，按照“问题-处理-确认人”记录。
- 生成 `docs/archive/development-plans/214-signoff-YYYYMMDD.md`，包含最终版本摘要、链接、签字人。
...
- `make db-migrate-all`、`make db-rollback-last`、`go test ./...` 均在 Go 1.24 环境下通过。
- `docs/archive/development-plans/214-signoff-*.md` 保存联合签字纪要。
- `docs/development-plans/210-database-baseline-reset-plan.md` 与 `06-integrated-teams-progress-log.md` 更新 Phase1 完成状态。

---

**版本历史**
- v1.0 (2025-11-05)：初稿，等待评审。  
- 后续更新：记录执行进展与调整。
