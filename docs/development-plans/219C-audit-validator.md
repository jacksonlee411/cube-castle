...
| 文档 | 审计/验证规则表（可为表格或 Markdown）、与 outbox/eventbus 的关系图 |
...
### 3.1 审计策略
1. **必审计操作清单**（需产出 `audit-rules.md` 或表格）：
...
2. 在 service 层统一调用 audit logger，确保与 outbox 同事务（必要时在事务内写入）。
...
2. 实现规则（需输出 `validator-rules.md` 覆盖表）：
...
3. 将 validator 注入 service 层，并在关键命令执行前调用；形成规则矩阵（规则→触发命令→严重级别）。

### 3.3 文档与脚本
1. 输出 `docs/development-plans/219C-audit-validator.md`（本文件）更新后的规则与说明。
2. 在 README 或 `docs/reference/` 中附上审计字段定义、validator 规则表。
3. 若涉及配置（如审计保留天数），在 `.env`/config 中说明。
