...
**状态**: 暂缓 · 准入条件：Plan 242 命名抽象（T2/T3/T4）完成且 Plan 244 验收通过后恢复  
...
- 明确职责边界：`TemporalEntityPage` 负责路由/适配与实体配置，`TemporalEntityLayout` 仅提供骨架与 Slot；避免双入口与内容重叠。  
- `TemporalEntityPage` 作为外层路由入口；组织/职位通过适配层以 props/slot 注入特定卡片与按钮，禁止复制布局。  
...
- 将 `usePositionDetail` 与 `useEnterpriseOrganizations`（导出 `useOrganizationDetail` 薄封装）统一代理到 `useTemporalEntityDetail`；通过守卫/ESLint 限制外部仅引用权威入口。  
...
- 更新所有职位/组织页面与 Playwright 用例引用该选择器；沿用 `scripts/quality/selector-guard-246.js` 守卫，禁止新增旧前缀。  
...
3. **Selector 稳定性**：`tests/e2e/position-tabs.spec.ts`、`frontend/tests/e2e/temporal-management-integration.spec.ts` 在 Chromium/Firefox 连续 3 次通过，且只引用 `temporalEntitySelectors`；`npm run guard:selectors-246` 与 `npm run guard:plan245` 均通过，且基线计数不升高。  
