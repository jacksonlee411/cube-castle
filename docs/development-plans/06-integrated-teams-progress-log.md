...
2. **文档与性能摘要**  
   ...
   - 219E 回退/Outbox 指标仍缺可引用的日志条目（取证 Runbook 见 `logs/219E/outbox-dispatcher-plan.md`，分析详见 `docs/development-plans/231-outbox-dispatcher-gap.md`；当前日志表明 outbox 未写入，需进一步整改）。
...
| Outbox/Dispatcher 指标验证 | 命令 + 平台团队 | ⏳ 进行中 | 2025-11-08 已执行 O1-O6（`logs/219E/outbox-dispatcher-events-20251108T112139.log`、`...-metrics-20251108T112459.log`、`...-sql-20251108T112236.log`、`...-run-20251108T112541.log`、`position-gql-outbox-20251108T112820.log`），结果显示 `outbox_events` 为空、dispatcher 指标仍为 0（详见 `docs/development-plans/231-outbox-dispatcher-gap.md`）；需确认命令链路是否写入 outbox，若缺失需补充修复后再次取证 | `docs/development-plans/231-outbox-dispatcher-gap.md`、`logs/219E/outbox-dispatcher-plan.md`、`scripts/219C3-rest-self-test.sh` |
