...
4. **Week 5 结束**（Phase C 完成） — ✅ 2025-10-16 完成  
   - 检查点：前端交互完成，E2E 测试通过  
   - 记录：职位详情页展示当前任职、任职历史、调动记录；`npx vitest run frontend/src/features/positions/__tests__/PositionDashboard.test.tsx` 通过；新增 Playwright 脚本 `frontend/tests/e2e/position-lifecycle.spec.ts`（待执行上传截图）  
   - 更新：06号日志「Phase C 验收」
