...
- **🚧 Plan 18 Phase 1.3**（2025-10-05）：迁移脚本 025a/027a/030 + 新增 031 完成，`make db-migrate-all` 全量执行通过（最新日志：`reports/iig-guardian/plan18-migration-20251005T113248.log`）；`scripts/plan18/run-business-flow-e2e.sh` 最新复测 9/10 通过，创建流程恢复正常（`plan18-business-flow-20251005T113248.log`），Firefox “错误处理与恢复” 剧本因“重试”按钮未出现 15s 超时，需更新 UI 或测试逻辑。验证报告：`reports/iig-guardian/plan18-phase1.3-validation-20251005.md`。
- **⚠️ Plan 18 阻塞项**（P0）：Firefox 错误恢复剧本仍失败；需确认错误态展示与“重试”按钮选择器。后续继续推进 Phase B 审计触发器整改（移除 `operation_reason`/`is_temporal` 依赖）。
