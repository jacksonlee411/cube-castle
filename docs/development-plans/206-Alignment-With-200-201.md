...
- `../archive/development-plans/205-HRMS-Transition-Plan.md`（过渡方案 - 已完成并归档）
- `200-Go语言ERP系统最佳实践.md`（参考）
- `201-Go实践对齐分析.md`（参考）
- **Phase1 执行状态**:
  - `210-database-baseline-reset-plan.md` ✅
  - `211-phase1-module-unification-plan.md` ✅
  - `212-shared-architecture-alignment-plan.md` ✅
  - `213-go-toolchain-baseline-plan.md` ✅
  - `214-phase1-baseline-extraction-plan.md` ✅

## 执行摘要

**状态更新（2025-11-04）**: Plan 205 已完成并归档至 `docs/archive/development-plans/`。Phase1 (Plans 210-214) 已全部完成，现进入 Phase2 (Plans 215-219) 执行阶段。

通过对200号文档（Go ERP最佳实践）和201号文档（项目现状分析）的深度审视，目前 **203/204 + 归档的 205 文档已经覆盖 7 个关键一致性维度**，重点转向执行落地与进度管控：

1. **数据访问层演进（sqlc/Ent）**：203 第4.5节、204 第三阶段与已归档的 205 步骤10均已纳入，待按阶段实施。
2. **异步可靠性（事务性发件箱）**：203 第4.3.3节明确 `event_id` 幂等策略，204/205 要求建立统一 outbox 基座（Plan 217B 正在实施）。
3. **数据库迁移治理（Atlas + Goose + Down 脚本）**：204 第二阶段新增 goose/down/atlas 交付物，已在 Plan 210 中实施完成。
4. **权限策略外部化（Casbin/配置化）**：203 第5.4节提供三阶段路线，需在实施计划中跟踪排期。
5. **测试策略（Docker 实库 + 集成测试）**：204 第二阶段交付 Docker 基座，已纳入 Plan 214 和 218C 验证。
6. **连接池配置一致性**：203 第10.1节与已归档的 205 步骤10 已明确参数基线（Plan 218A-D 中已执行）。
7. **架构形态评估（模块化单体 vs. 多服务）**：203/204 已给出评估路径，需在执行期持续复盘。

后续工作重点：
- 按 204 时间表跟进 `sqlc` 试点（Plan 217）、outbox relay（Plan 217B）、Docker 集成测试的实际完成情况。
- 在 Phase2 验收流程中强制执行 `make sqlc-generate`、`make db-migrate-verify`、`make test-db`。
...
- `../archive/development-plans/205-HRMS-Transition-Plan.md:300-360,686-711` 通过步骤10、测试清单和命令速查表将 `make sqlc-generate`、`git diff --exit-code` 列为强制项。
- **Plan 217** (Database Layer Implementation) 正在逐步实施 sqlc 迁移。
...
- `../archive/development-plans/205-HRMS-Transition-Plan.md:330-360,370-432,726-757` 指导创建共享 outbox 组件、relay、Prometheus 指标，并在验收清单中验证。
- **Plan 217B** (Outbox Dispatcher Plan) 已制定具体实施方案。
...
- `../archive/development-plans/205-HRMS-Transition-Plan.md:272-360,686-711` 在步骤10和测试清单中，将 `make db-migrate-verify` 作为合并前必跑项。
- **Plan 210** (Database Baseline Reset Plan) ✅ 已完成实施。
...
- `../archive/development-plans/205-HRMS-Transition-Plan.md:330-360` 要求在共享 `internal/` 中集中权限装载逻辑，便于替换为配置或 Casbin。
- **权限外部化**仍在评估阶段，未纳入 21X 计划。
...
- `../archive/development-plans/205-HRMS-Transition-Plan.md:360-432,686-711` 新增 Docker 测试基座步骤、风险条目与验收清单（`make test-db`）。
- **Plan 214** (Phase1 Baseline Extraction) ✅ 已完成 Docker 测试基座建立。
- **Plan 218C** (Logger Verification Report) ✅ 已验证缓存层测试覆盖。
...
- `../archive/development-plans/205-HRMS-Transition-Plan.md:300-336,700-757` 步骤10和验收清单要求显式设置连接池并通过 Prometheus 监控。
- **Plan 218A-D** (Logger Migration) ✅ 已在命令服务中实施连接池配置。
...
**Phase1 执行完成（Plans 210-214）** ✅
- ✅ `210-database-baseline-reset-plan.md` - 已完成数据库基线重建
- ✅ `211-phase1-module-unification-plan.md` - 已完成模块统一化
- ✅ `212-shared-architecture-alignment-plan.md` - 已完成架构对齐
- ✅ `213-go-toolchain-baseline-plan.md` - 已完成工具链基线
- ✅ `214-phase1-baseline-extraction-plan.md` - 已完成基线提取

**204 (路线图) 现状**：
- ✅ 第二阶段加入 down.sql/atlas/Docker 基座
- ✅ 第三阶段纳入 sqlc、outbox、`make test-db` 验收
- 🔄 尚需在后续版本明确权限外部化与架构复盘的具体里程碑（建议纳入 Q1 计划）

**205 (已归档) 现状**：
- ✅ 已完成并归档至 `docs/archive/development-plans/`
- ✅ 步骤10覆盖连接池、sqlc、outbox、Docker 集成测试
- ✅ 验收清单新增 `make sqlc-generate`/`make db-migrate-verify`/`make test-db`
- 🔄 后续版本应补充权限外部化和 Casbin POC 的操作指引（该部分内容已保留在归档版本中）

**Phase2 执行进行中（Plans 215-219）**：
- 🔄 `215-phase2-execution-log.md` / `215-phase2-summary-overview.md` - 进行中
- 🔄 `216-eventbus-implementation-plan.md` - 进行中
- 🔄 `217-database-layer-implementation.md` / `217B-outbox-dispatcher-plan.md` - 进行中
- ✅ `218-logger-system-implementation.md` / `218A-D` - 已完成
- 🔄 `219-organization-restructuring.md` / `219A-E` - 进行中
...
| 维度 | 200号文档 | 201号现状 | 203/204/归档的205 现状 | 对齐度 | 后续跟踪 |
|------|----------|---------|-------------------|--------|---------|
| **架构形态** | 模块化单体优先 | 已拆分两服务 | 203 §7/204 第一阶段规划合并路径 | ⚠️ 60% | 评估单进程收敛可行性 |
| **数据访问** | sqlc/Ent 编译期安全 | 手写SQL + 手动 Scan | ✅ 203 §4.5 + 204 Stage3 + Plan 217 试点 | ⚠️ 75% | 落地 workforce sqlc 试点 |
| **异步可靠性** | 事务性发件箱必须 | 纯内存队列 | ✅ 203 §4.3.3 + 204 Stage2 + Plan 217B 方案 | ⚠️ 75% | 实现 outbox/relay 与监控 |
| **迁移回滚** | Atlas+Goose 工作流 | 无 down.sql | ✅ 203 附录D + 204 Stage2 + Plan 210 完成 | ✅ 85% | 补齐历史 down.sql 并启用 CI |
| **权限策略** | Casbin/配置外部化 | 硬编码 Go map | ✅ 203 §5.4（路线） | ⚠️ 60% | 制定外部化里程碑与 POC |
| **数据测试** | Docker 实库集成测试 | 主要依赖 sqlmock | ✅ 203 §8.2 + 204 Stage2 + Plan 214/218C 验证 | ✅ 85% | 扩大 `make test-db` 覆盖范围 |
| **连接池** | 显式配置/监控 | 查询✅/命令❌ | ✅ 203 §10.1 + Plan 218A-D 完成 | ✅ 85% | 实现 Prometheus 告警 |
| **API 管理** | 先契约后实现 | 已实现 | ✅ 203 §5 + 契约文档 | ✅ 100% | 持续维持 |
| **模块划分** | DDD 界定上下文 | N/A | ✅ 203 第2章 | ✅ 100% | 持续维持 |

**总体对齐度**: 95%（文档已对齐） + 75%（执行进度） = **~85% 整体落地** → 重点转为 Phase2 执行跟踪与验收
