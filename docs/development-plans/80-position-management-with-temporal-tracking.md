...
- [ ] 产出职位管理导航结构、列表/详情/编制看板的线框图或 Figma 设计
- [ ] 与组织模块共享 UI 组件清单，确定复用/新增组件范围
- [ ] 准备 mock 数据（JSON/fixtures），覆盖多编制、未来版本、空缺等场景
- [ ] 召开设计评审，确保业务方确认布局与信息密度

**Week 0-2: React Mock 页面（无真实 API）**
- [ ] 基于 mock 数据搭建职位列表/详情/表单骨架
- [ ] 实现导航、筛选器、职位编制卡片等核心布局，与实际数据逻辑解耦
- [ ] 编写 Storybook/Vitest 组件测试，确保后续替换数据不影响结构
