...
- golangci-lint run
  ...
  - tagliatelle（新增）：JSON tag 必须为 camelCase（白名单：OAuth/OIDC/JWT 与 auth 目录；见本计划“例外清单”）
...
## 6. 决议与例外（针对开放问题）
- OAuth/OIDC/JWT 标准字段例外（snake_case 允许）：access_token、refresh_token、token_type、id_token、expires_in、authorization_endpoint、token_endpoint、end_session_endpoint、jwks_uri、tenant_id（JWT claim）
  - 约束：仅限认证/会话相关包路径（internal/auth/**、cmd/hrms-server/**/auth/**、cmd/hrms-server/**/authbff/**），其它领域严禁 snake_case 用于对外响应。
- JSON 命名守卫策略（软→硬）：
...
- 端口检测边界：255 仅覆盖“代码层硬编码端口/直连 :9090|:8090|localhost”与“禁止 GET 直连查询”；Compose 端口映射/镜像标签/冷启动由 253 负责。
...
- 新增工作流 .github/workflows/plan-255-gates.yml（不复制规范，作为执行器与证据归档）：
  ...
  - 依赖：将 plan-253-gates 设为仓库受保护分支的必需检查之一（组合门禁）

---

## 8. 验收标准（更新版）
- 前端门禁：architecture-validator（cqrs,ports,forbidden）关键违规为 0；禁止 fetch GET/axios.get 查询；禁止直连 :9090/:8090
- 后端门禁：
  - depguard：command↔query 内部依赖为 0
  - tagliatelle：除白名单外 json tag snake_case 为 0（软→硬策略按迭代执行）
- 证据：logs/plan255/* 与 reports/architecture/architecture-validation.json 存在，215 已登记
- 组合门禁：plan-253-gates 通过（compose 端口映射与镜像标签）
...
- M2：启用 tagliatelle（软门禁）；登记与收敛首批 snake_case；更新 215 日志
