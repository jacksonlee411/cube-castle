...
### 核心问题（2025-10-08 更新）
- **Go后端**: 红灯已清零；仓储拆分为 5 个模块（最大 475 行）。
- **前端TS**: 红灯已清零；`InlineNewVersionForm` 拆分为容器 + 子组件/动作工厂（单文件 ≤150 行，核心 hook 232 行）。
- **待收尾**: Playwright RS256 复测失败用例（GraphQL 契约与性能问题）仍需闭环。

### 核心目标
- **Phase 1（执行中）**: Go 红灯 3→0、TS 红灯 2→0；待补 Playwright RS256 回归证据。
...
## ✅ 最新进展（2025-10-08 更新）
- 查询服务 `main.go` 拆分为 app/model/repository/graphql 等模块，`go test ./cmd/organization-query-service/...` 通过。
- 命令处理器 `organization.go` 拆分为 create/update/events/routes/helpers 模块，`go test ./cmd/organization-command-service/...` 通过。
- 命令服务仓储/时间轴巨石文件已拆分为 11 个子模块（最大 217 行）。
- 查询服务仓储巨石 (`postgres.go`) 已拆分为 `postgres_base.go` 等 5 个文件（最大 475 行），Go 红灯已清零。
- `.golangci.yml` 启用 depguard，`go vet ./...` 与 `make test-integration` 已完成（2025-10-07，E2E_RUN 未设步跳过真实 HTTP）。
- Temporal 主视图拆分：`TemporalMasterDetailView.tsx` 已降至 380 行（脱离红灯），新增 `TemporalMasterDetailHeader.tsx`（101 行）、`TemporalMasterDetailAlerts.tsx`（101 行）、`hooks/useTemporalMasterDetail.ts`（521 行）、`hooks/temporalMasterDetailApi.ts`（289 行）。
- `InlineNewVersionForm` 拆分为容器组件、8 个子组件、独立 `useInlineNewVersionForm` hook 与 `formActions` 工厂；最大文件 232 行，前端红灯清零。
- GraphQL 测试脚本（e2e + consistency + Redis 缓存）已对齐 `OrganizationConnection` 契约：统一使用 `PaginationInput` 与 `filter.codes`，验证逻辑读取 `pagination.total`。
- Batch A（shared/api）弱类型治理完成：`unknown/any` 由 74 处降至 6 处（详见 `code-smell-types-20251007.md` 后附复核），OAuth/GraphQL 客户端现具备显式 DTO。
- TypeScript 弱类型统计基线：`reports/iig-guardian/code-smell-types-20251007.md` 记录 173 处 `any/unknown`。
- 集成测试证据已追加至 `reports/iig-guardian/code-smell-progress-20251007.md`。
...
#### 技术方案可行性
- [x] **文件拆分策略合理**: 手动验证 main.go/organization.go 已拆分为多模块（最大 217 行）。
- [x] **重构方法可行**: 查询服务与命令处理器已按组件/仓储划分；go test 通过。
- [ ] **测试策略充分**: 集成测试已执行（2025-10-07），Playwright RS256 复测待补。
- [x] **回滚机制明确**: `plan16-phase0-baseline` + 每阶段标签已建立。

#### 风险评估充分性
- [x] **高风险已识别**: 后端拆分阶段已执行 go test 验证；前端拆分待 Phase 1.5。
- [x] **预防措施到位**: 每次拆分已运行 `go test`，CQRS 依赖通过 `.golangci.yml` 限制。
- [x] **响应对策清晰**: 回滚标签保持有效；暂无回滚记录。
...
- ✅ 实现清单刷新与临时治理巡检（`reports/implementation-inventory.json`、`reports/iig-guardian/todo-temporary-ci-verification-20251003.md`）
- ✅ `.golangci.yml` depguard 配置与 `go vet ./...` 已完成（2025-10-07）
- ⏳ 待完成：`plan16-phase0-baseline` 标签推送、工作量复核纪要、06号日志责任人补录、Playwright RS256 复测记录

### 立即行动清单（批准当天 ~45分钟）
```bash
# （已于 2025-09-30 执行完毕，以下命令保留作复查脚本）
# 1. 运行实现清单（30分钟）
...
### 批准后1天内完成
- 工作量复核会议（1小时，⏳ 纪要待归档）
- `git tag plan16-phase0-baseline` 并推送远端（⏳ 待执行）
- 更新 `docs/development-plans/06-integrated-teams-progress-log.md`（⏳ 责任人仍待补录）
- 运行 Phase 0 验收检查脚本（⏳ 待补日志证明）
```bash
bash <<'EOF'
echo "=== Phase 0 验收检查 ==="
...
| 指标 | 基线（2025-09-29） | 当前进展（2025-10-08） | 目标（2025-11-08） |
|------|-------------------|---------------------|-------------------|
| Go红灯文件 | 3个（27.5%代码） | **0个**（仓储拆分完成，最大 475 行） | 0个 |
| TS红灯文件 | 2个（12.2%代码） | **0个**（Inline 表单重构完成） | 0个 |
| TS平均行数 | 163.0行 | 163.0行（待 IIG 刷新，Inline 表单拆分完成） | ≤150行 |
| Go橙灯文件 | 5个 | 0个 | ≤3个 |
| TS橙灯文件 | 9个 | 9个 | ≤5个 |
| any/unknown使用 | 171处 | 171处 | ≤30处 |
| 单元测试覆盖率 | 当前水平 | `go test` 已通过 | ≥80% |
| 集成/契约测试 | `go vet ./...`、`make test-integration` 完成（2025-10-07） | Playwright RS256 待复测 | 100% 通过 |
