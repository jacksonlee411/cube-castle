...
- **实施清单**：
  ...
  | 4 | 文档/速查 | 更新 `internal/standardobject/README.md`、`docs/reference/01-DEVELOPER-QUICK-REFERENCE.md`，强调组织/职位写路径仅走 SOM，并提供 `logs/plan402/capability/port-injection.log` 样例。 |
...
- **执行步骤**：
  1. `make db-migrate-all && go run ./cmd/tools/standardobject-migrator --from legacy --to som --log-file logs/plan402/migration/migrator-$(date +%Y%m%d%H%M).log`，迁移选项需包含批次/重试配置。
  2. 将旧仓储设为只读：例如通过 SQL `REVOKE INSERT,UPDATE,DELETE ON organization_units FROM app_user;` 并在命令服务中删除对应写分支。
  3. 运行 `go run ./cmd/tools/standardobject-validator --log-file logs/plan402/validator/validator-$(date +%Y%m%d%H%M).json --time-constraint-report logs/plan402/migration/time-constraint-report.log --transaction-gap logs/plan402/migration/transaction-gap.log`。
  4. 若有差异，复用 migrator 的 `--fixup` 模式产出补丁，再次执行步骤 3 至 PASS；不得以“允许差异”“白名单”形式跳过。
...
- Outbox dispatcher 仍由 PostgreSQL 事务性发件箱驱动，消费者使用 Redis/Asynq（`pkg/eventbus/asynq`）落地；快照刷新任务只允许由该持久化队列触发（禁止在生产路径中调用内存队列/`async.Enqueue`）。Asynq 配置文件（`configs/eventbus.standard-object.yaml`）必须记录队列名、重试策略、dead-letter 处理与 `docker compose exec <svc> asynqmon` 诊断路径。
...
| 风险 | 影响 | 缓解措施 |
|------|------|----------|
| 迁移遗漏 / 校验不充分 | 数据进入 SOM 后仍残留旧表差异 | 使用 migrator + validator 一次性完成导入与校验，`time-constraint-report.log`、`transaction-gap.log` 必须无异常；发现差异立即通过 migrator `--fixup` 重跑，禁止带差上线 |
