...
- 219C3 新增：`scripts/219C3-rest-self-test.sh` 聚焦 REST 命令自测（Position Create/Fill/Close + Job Level Version），输出 `logs/219C3/validation.log` / `report.json`，并在设置 `DATABASE_URL` 时自动查询 `audit_logs` 验证 `ruleId` / `severity`。
- 219C3A 更新：`internal/organization/handler/position_handler.go` 将业务规则失败统一写入审计（`audit.LogError`），同时扩展 `writeValidationFailure` 结构化 `ruleId`、`severity`；配套单测 `position_handler_test.go` 覆盖。
- 219C3A 更新：`internal/organization/repository/job_catalog_repository.go` 修复 `InsertJobLevelVersion` 父版本字段复用及 `is_current` 逻辑，新增 `job_catalog_repository_test.go` 以 SQLMock 覆盖成功 / parent mismatch 场景。
...
- 事务性发件箱：
  ...
  - 事件构建：`internal/organization/events/outbox.go` 定义统一 `Context` 结构与 Assignment/Position/JobLevel 事件 helper（payload 固定包含 `tenantId`、`requestId`、`correlationId`、`operation`、`source`、`occurredAt`、`aggregateId/Type`，并追加领域字段如 `positionCode`、`assignmentId`、`jobLevelCode`、`headcountInUse` 等）。
