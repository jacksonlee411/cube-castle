...
func (r *PostgreSQLRepository) GetAuditHistory(ctx context.Context, tenantId uuid.UUID, recordId string, startDate, endDate, operation, userId *string, limit int) ([]dto.AuditRecordData, error) {
	start := time.Now()
	...
	addOptional("startDate", startDate)
...
func (r *PostgreSQLRepository) processAuditRowsLegacy(rows *sql.Rows) ([]dto.AuditRecordData, error) {
	...
	for rows.Next() {
		...
		record.OperatedByField = dto.OperatedByData{
...
func (r *PostgreSQLRepository) processAuditRowsStrict(rows *sql.Rows) ([]dto.AuditRecordData, error) {
...
func (r *PostgreSQLRepository) GetPositionTransfers(ctx context.Context, tenantID uuid.UUID, positionCode *string, organizationCode *string, pagination *dto.PaginationInput) (*dto.PositionTransferConnection, error) {
	page := int32(1)
	pageSize := int32(25)
	if pagination != nil {
...
`, baseCTE, filterClause, limitIdx, offsetIdx)
	...
	edges := make([]dto.PositionTransferEdge, 0, len(transfers))
	...
	connection := &dto.PositionTransferConnection{
		EdgesField: edges,
		DataField:  transfers,
...
func scanPositionTransfer(scanner rowScanner) (*dto.PositionTransfer, error) {
	var (
		transferID    string
		positionCode  string
		fromOrg       string
...
// 单条审计记录查询 - v4.6.0
func (r *PostgreSQLRepository) GetAuditLog(ctx context.Context, auditId string) (*dto.AuditRecordData, error) {
	start := time.Now()
	...
	if err != nil {
		...
		log.WithFields(pkglogger.Fields{"error": err}).Error("audit log query failed")
		return nil, err
	}
...
func sanitizeChanges(raw string) ([]dto.FieldChangeData, []string, error) {
	trimmed := strings.TrimSpace(raw)
	if trimmed == "" {
	...
	sanitized := make([]dto.FieldChangeData, 0, len(rawArray))
	issues := make([]string, 0)
