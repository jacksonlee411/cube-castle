...
func (r *PostgreSQLRepository) GetOrganizationStats(ctx context.Context, tenantID uuid.UUID) (*dto.OrganizationStats, error) {
	start := time.Now()
	...
	stats.TemporalStatsField = dto.TemporalStats{
...
func (r *PostgreSQLRepository) GetOrganizationHierarchy(ctx context.Context, tenantID uuid.UUID, code string) (*dto.OrganizationHierarchyData, error) {
	start := time.Now()
...
func (r *PostgreSQLRepository) GetOrganizationSubtree(ctx context.Context, tenantID uuid.UUID, code string, maxDepth int) (*dto.OrganizationHierarchyData, error) {
	start := time.Now()
	...
	for rows.Next() {
		...
		node.ChildrenField = []dto.OrganizationSubtreeData{}
...
func convertSubtreeToHierarchy(node *dto.OrganizationSubtreeData) *dto.OrganizationHierarchyData {
	if node == nil {
		return nil
	}

	children := make([]dto.OrganizationHierarchyData, 0, len(node.ChildrenField))
