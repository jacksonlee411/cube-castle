...
func TestAuthFlow_RealHTTP_RS256_JWKS_and_TenantChecks(t *testing.T) {
    ...
    if _, err := os.Stat(pkPath); os.IsNotExist(err) {
        ...
        if err != nil { t.Fatalf("generate rsa: %v", err) }
        b := x509.MarshalPKCS1PrivateKey(key)
        pemBytes := pem.EncodeToMemory(&pem.Block{Type: "RSA PRIVATE KEY", Bytes: b})
        if err := os.WriteFile(pkPath, pemBytes, 0o600); err != nil { t.Fatalf("write pem: %v", err) }
    ...
    if err != nil { t.Fatalf("session failed: %v", err) }
    var sessBody map[string]any
    if err := json.NewDecoder(resp.Body).Decode(&sessBody); err != nil { t.Fatalf("decode session: %v", err) }
    _ = resp.Body.Close()
    ...
    if err != nil { t.Fatalf("graphql request failed: %v", err) }
    if resp.StatusCode != 200 { t.Fatalf("graphql expected 200, got %d", resp.StatusCode) }
    _ = resp.Body.Close()
    ...
    if err != nil { t.Fatalf("graphql mismatch failed: %v", err) }
    if resp.StatusCode != 403 { t.Fatalf("graphql expected 403 on tenant mismatch, got %d", resp.StatusCode) }
    _ = resp.Body.Close()
    ...
    for _, c := range jar.Cookies(u) {
        if c.Name == "csrf" { csrf = c.Value; break }
    }
    if csrf == "" { t.Fatalf("missing csrf cookie") }
    ...
    if resp.StatusCode != 200 { t.Fatalf("expected 200 refresh, got %d", resp.StatusCode) }
    _ = resp.Body.Close()
}
