...
### 当前系统状况
- ✅ **后端实体**: `OrganizationUnit`完整实现，支持层级结构
- ✅ **适配器层**: `OrganizationAdapter`提供前端兼容性
- ✅ **双路径支持**: `/organization-units` 和 `/corehr/organizations`
- ✅ **多态配置**: 基于`unit_type`的`profile`字段动态配置
- ✅ **层级管理**: 自引用`parent_unit_id`实现组织树
