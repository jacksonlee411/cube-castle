...
--
-- Name: enhanced_gap_fill(character varying); Type: FUNCTION; Schema: public; Owner: user
--

CREATE FUNCTION public.enhanced_gap_fill(p_org_code character varying) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    ...
    timeline_start DATE;
    timeline_end DATE;
    current_coverage DATE;
    best_extender RECORD;
    fill_success BOOLEAN := true;
    operations_count INTEGER := 0;
    audit_message TEXT := '';
BEGIN
    -- 第1步：获取所有有效记录（ACTIVE状态，排除作废记录）
    CREATE TEMP TABLE IF NOT EXISTS valid_timeline AS
    SELECT code, name, effective_date, end_date, tenant_id, unit_type, 
           parent_code, status, level, path, sort_order, description, 
    ...
    SELECT MIN(effective_date), 
           COALESCE(MAX(CASE WHEN end_date IS NULL THEN '2099-12-31'::DATE ELSE end_date END), '2099-12-31'::DATE)
    INTO timeline_start, timeline_end
    FROM valid_timeline;
    
    -- 如果没有有效记录，直接返回
    IF timeline_start IS NULL THEN
        INSERT INTO temporal_gap_audit (organization_code, gap_type, fill_success, audit_message, created_at)
        VALUES (p_org_code, 'NO_ACTIVE_RECORDS', false, '无有效记录需要填充', NOW());
        RETURN false;
    END IF;
    
    -- 第3步：从时间轴起点开始，逐段检查并填充
    current_coverage := timeline_start;
    
    WHILE current_coverage < timeline_end LOOP
        -- 查找能覆盖当前时间点的最佳记录
        SELECT * INTO best_extender
        FROM valid_timeline
        WHERE effective_date <= current_coverage
        AND (end_date IS NULL OR end_date >= current_coverage)
        ORDER BY effective_date DESC  -- 选择最近的记录
        LIMIT 1;
        
        -- 如果找不到覆盖记录，寻找最近的前置记录进行扩展
        IF best_extender IS NULL THEN
            SELECT * INTO best_extender
            FROM valid_timeline  
            WHERE effective_date <= current_coverage
            ...
            -- 扩展这条记录到当前需要覆盖的时间点
            IF best_extender IS NOT NULL THEN
                -- 寻找下一个记录的开始时间作为扩展终点
                SELECT COALESCE(MIN(effective_date) - INTERVAL '1 day', timeline_end)
                ...
                SET end_date = current_coverage::DATE,
                    updated_at = NOW()
                WHERE code = p_org_code
                AND effective_date = best_extender.effective_date;
                
                operations_count := operations_count + 1;
                audit_message := audit_message || format('扩展 %s 至 %s; ', best_extender.name, current_coverage);
            END IF;
        ELSE
            -- 找到覆盖记录，移动到其覆盖范围的下一个时间点
            current_coverage := COALESCE(best_extender.end_date, timeline_end) + INTERVAL '1 day';
        END IF;
        
        -- 防止无限循环
        IF operations_count > 50 THEN
            EXIT;
        END IF;
    END LOOP;
    
    -- 记录审计日志
    INSERT INTO temporal_gap_audit (
        organization_code, gap_type, fill_strategy, fill_success,
        audit_message, created_at
    ) VALUES (
        p_org_code, 'FULL_TIMELINE_REBUILD', 'INTELLIGENT_EXTENSION',
        fill_success, 
        format('全时间轴重建完成，执行%s次操作：%s', operations_count, audit_message),
        NOW()
    );
    
    -- 清理临时表
    DROP TABLE IF EXISTS valid_timeline;
...
EXCEPTION
    WHEN OTHERS THEN
        DROP TABLE IF EXISTS valid_timeline;
        INSERT INTO temporal_gap_audit (
            organization_code, gap_type, fill_success, audit_message, created_at
        ) VALUES (
            p_org_code, 'ERROR', false, 
            format('全时间轴重建失败: %s', SQLERRM), NOW()
        );
        RETURN false;
END;
$$;
...
ALTER FUNCTION public.enhanced_gap_fill(p_org_code character varying) OWNER TO "user";
...
--
-- Name: get_organization_temporal(uuid, character varying, date); Type: FUNCTION; Schema: public; Owner: user
--
...
ALTER FUNCTION public.get_organization_temporal(p_tenant_id uuid, p_code character varying, p_as_of_date date) OWNER TO "user";
...
--
-- Name: simple_gap_fill(character varying, date, date); Type: FUNCTION; Schema: public; Owner: user
--

CREATE FUNCTION public.simple_gap_fill(p_org_code character varying, p_deactivated_effective_date date, p_deactivated_end_date date) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    prev_record RECORD;
    next_record RECORD;
    fill_success BOOLEAN := false;
    audit_message TEXT;
BEGIN
    -- 查找前一条相邻记录
    SELECT code, effective_date, end_date, name, tenant_id, unit_type, parent_code, status, level, path, sort_order, description, is_current
    INTO prev_record
    FROM organization_units
    WHERE code = p_org_code
    AND status = 'ACTIVE'
    AND end_date = p_deactivated_effective_date - INTERVAL '1 day'
    LIMIT 1;
    ...
    WHERE code = p_org_code
    AND status = 'ACTIVE'
    AND effective_date = p_deactivated_end_date + INTERVAL '1 day'
    LIMIT 1;
    
    -- 执行填充逻辑
    IF prev_record IS NOT NULL THEN
        -- 前向扩展策略：扩展前一条记录到被作废记录的结束日期
        UPDATE organization_units 
        SET 
            end_date = p_deactivated_end_date,
            updated_at = NOW()
        WHERE code = p_org_code
        AND effective_date = prev_record.effective_date;
        
        audit_message := format('前向扩展成功: %s 从 %s 扩展至 %s', 
            prev_record.name, prev_record.end_date, p_deactivated_end_date);
        fill_success := true;
        
    ELSIF next_record IS NOT NULL THEN
        -- 后向扩展策略：删除旧记录，插入新记录（前移开始日期）
        DELETE FROM organization_units
        WHERE code = p_org_code AND effective_date = next_record.effective_date;
        
        INSERT INTO organization_units (
            tenant_id, code, name, unit_type, parent_code, status, level, path,
            sort_order, description, effective_date, end_date, is_current,
            created_at, updated_at
        ) VALUES (
            next_record.tenant_id, next_record.code, next_record.name, 
            next_record.unit_type, next_record.parent_code, next_record.status,
            next_record.level, next_record.path, next_record.sort_order, 
            next_record.description, p_deactivated_effective_date, next_record.end_date,
            next_record.is_current, NOW(), NOW()
        );
        
        audit_message := format('后向扩展成功: %s 从 %s 前移至 %s', 
            next_record.name, next_record.effective_date, p_deactivated_effective_date);
        fill_success := true;
        
    ELSE
        audit_message := '无相邻记录可扩展，需要手动处理';
        fill_success := false;
    END IF;
    
    -- 记录审计日志
    INSERT INTO temporal_gap_audit (
        organization_code, gap_type, deactivated_period, fill_strategy,
        fill_success, audit_message, created_at
    ) VALUES (
        p_org_code,
        CASE 
            WHEN prev_record IS NOT NULL AND next_record IS NOT NULL THEN 'MIDDLE_GAP'
            WHEN prev_record IS NOT NULL AND next_record IS NULL THEN 'END_GAP'
            WHEN prev_record IS NULL AND next_record IS NOT NULL THEN 'START_GAP'
            ELSE 'ISOLATED_GAP'
        END,
        format('%s to %s', p_deactivated_effective_date, p_deactivated_end_date),
        CASE 
            WHEN prev_record IS NOT NULL THEN 'EXTEND_PREVIOUS'
            WHEN next_record IS NOT NULL THEN 'EXTEND_NEXT'
            ELSE 'MANUAL_REQUIRED'
        END,
        fill_success, audit_message, NOW()
    );
...
EXCEPTION
    WHEN OTHERS THEN
        INSERT INTO temporal_gap_audit (
            organization_code, gap_type, fill_success, 
            audit_message, created_at
        ) VALUES (
            p_org_code, 'ERROR', false,
            format('填充失败: %s', SQLERRM), NOW()
        );
        RETURN false;
END;
$$;
...
ALTER FUNCTION public.simple_gap_fill(p_org_code character varying, p_deactivated_effective_date date, p_deactivated_end_date date) OWNER TO "user";

--
-- Name: smart_timeline_fill(character varying); Type: FUNCTION; Schema: public; Owner: user
--

CREATE FUNCTION public.smart_timeline_fill(p_org_code character varying) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
DECLARE
    gap_record RECORD;
    ...
    audit_messages TEXT := '';
BEGIN
    -- 找到所有时间空洞：相邻ACTIVE记录之间的空隙
    FOR gap_record IN (
        WITH active_timeline AS (
            SELECT effective_date, 
                   COALESCE(end_date, '2099-12-31'::DATE) as end_date,
                   name, tenant_id, unit_type, parent_code, level, path, 
                   sort_order, description, is_current, created_at, updated_at
            FROM organization_units 
            WHERE code = p_org_code AND status = 'ACTIVE'
            ORDER BY effective_date
        ),
        gaps AS (
            SELECT 
                t1.end_date + INTERVAL '1 day' as gap_start,
                t2.effective_date - INTERVAL '1 day' as gap_end,
                t1.name as prev_name,
                t1.effective_date as prev_start,
                t1.end_date as prev_end,
                t2.name as next_name,
                t2.effective_date as next_start,
                t2.end_date as next_end
            FROM active_timeline t1
            JOIN active_timeline t2 ON t2.effective_date > t1.end_date + INTERVAL '1 day'
            WHERE NOT EXISTS (
                SELECT 1 FROM active_timeline t3 
                WHERE t3.effective_date > t1.end_date 
                AND t3.effective_date < t2.effective_date
            )
        )
        SELECT * FROM gaps
    ) LOOP
        -- 选择扩展策略：优先扩展前一个记录
        UPDATE organization_units 
        SET end_date = gap_record.next_start - INTERVAL '1 day',
            updated_at = NOW()
        WHERE code = p_org_code 
        AND effective_date = gap_record.prev_start
        AND status = 'ACTIVE';
        
        operations_count := operations_count + 1;
        audit_messages := audit_messages || format('填充空洞：扩展 %s 从 %s 到 %s; ',
            gap_record.prev_name, gap_record.prev_end, gap_record.next_start - INTERVAL '1 day');
    END LOOP;
    ...
    INSERT INTO temporal_gap_audit (
        ...
        p_org_code, 'SMART_TIMELINE_FILL', 'EXTEND_TO_NEXT',
        true, 
        CASE WHEN operations_count > 0 
             THEN format('智能填充完成，执行%s次操作：%s', operations_count, audit_messages)
             ELSE '时间轴已连贯，无需填充'
        END,
        NOW()
    );
...
ALTER FUNCTION public.smart_timeline_fill(p_org_code character varying) OWNER TO "user";

--
-- Name: temporal_gap_auto_fill_trigger(); Type: FUNCTION; Schema: public; Owner: user
--

CREATE FUNCTION public.temporal_gap_auto_fill_trigger() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
    fill_result BOOLEAN := false;
    trigger_action VARCHAR := TG_OP;
BEGIN
    -- 只处理ACTIVE记录被作废或删除的情况
    IF (trigger_action = 'UPDATE' AND OLD.status = 'ACTIVE' AND NEW.status = 'INACTIVE') OR
       (trigger_action = 'DELETE' AND OLD.status = 'ACTIVE') THEN
        
        -- 尝试简单间隙填充
        SELECT simple_gap_fill(OLD.code, OLD.effective_date, OLD.end_date) 
        INTO fill_result;
        ...
        INSERT INTO temporal_gap_audit (
            ...
            OLD.code,
            'TRIGGER_EXECUTION',
            format('%s [%s to %s]', trigger_action, OLD.effective_date, OLD.end_date),
            'AUTO_TRIGGER',
            fill_result,
            format('触发器执行 %s 操作，自动填充结果: %s', trigger_action, 
                CASE WHEN fill_result THEN '成功' ELSE '失败或需要手动处理' END),
            NOW()
        );
...
EXCEPTION
    WHEN OTHERS THEN
        ...
        INSERT INTO temporal_gap_audit (
            ...
            COALESCE(OLD.code, 'UNKNOWN'), 'TRIGGER_ERROR', false,
            format('触发器执行异常: %s', SQLERRM), NOW()
        );
...
ALTER FUNCTION public.temporal_gap_auto_fill_trigger() OWNER TO "user";
...
--
-- Name: validate_temporal_consistency_v2(); Type: FUNCTION; Schema: public; Owner: user
--

CREATE FUNCTION public.validate_temporal_consistency_v2() RETURNS TABLE(organization_code character varying, issue_type character varying, description text)
    LANGUAGE plpgsql
    AS $$
BEGIN
    -- 检查当前版本标记唯一性
...
ALTER FUNCTION public.validate_temporal_consistency_v2() OWNER TO "user";
...
CREATE TABLE public.organization_units (
    ...
    CONSTRAINT check_temporal_dates CHECK ((effective_date <= COALESCE(end_date, '9999-12-31'::date))),
...
--
-- Name: organization_current; Type: VIEW; Schema: public; Owner: user
--
...
ALTER VIEW public.organization_current OWNER TO "user";

--
-- Name: organization_events; Type: TABLE; Schema: public; Owner: user
--
...
CREATE TABLE public.organization_events (
    ...
    CONSTRAINT chk_event_type CHECK (((event_type)::text = ANY ((ARRAY['CREATE'::character varying, 'UPDATE'::character varying, 'RESTRUCTURE'::character varying, 'DISSOLVE'::character varying, 'ACTIVATE'::character varying, 'DEACTIVATE'::character varying])::text[])))
);


ALTER TABLE public.organization_events OWNER TO "user";
...
--
-- Name: organization_units_backup_pre_temporal; Type: TABLE; Schema: public; Owner: user
--

CREATE TABLE public.organization_units_backup_pre_temporal (
...
ALTER TABLE public.organization_units_backup_pre_temporal OWNER TO "user";

--
-- Name: organization_units_backup_pre_version_removal; Type: TABLE; Schema: public; Owner: user
--

CREATE TABLE public.organization_units_backup_pre_version_removal (
    code character varying(10),
    parent_code character varying(10),
    tenant_id uuid,
    name character varying(255),
...
ALTER TABLE public.organization_units_backup_pre_version_removal OWNER TO "user";

--
-- Name: organization_units_backup_temporal; Type: TABLE; Schema: public; Owner: user
--
...
ALTER TABLE public.organization_units_backup_temporal OWNER TO "user";
...
--
-- Name: temporal_gap_audit; Type: TABLE; Schema: public; Owner: user
--

CREATE TABLE public.temporal_gap_audit (
    id integer NOT NULL,
    organization_code character varying(50) NOT NULL,
    gap_type character varying(50),
    deactivated_period character varying(100),
    fill_strategy character varying(50),
...
ALTER TABLE public.temporal_gap_audit OWNER TO "user";

--
-- Name: temporal_gap_audit_id_seq; Type: SEQUENCE; Schema: public; Owner: user
--

CREATE SEQUENCE public.temporal_gap_audit_id_seq
    AS integer
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
...
ALTER SEQUENCE public.temporal_gap_audit_id_seq OWNER TO "user";

--
-- Name: temporal_gap_audit_id_seq; Type: SEQUENCE OWNED BY; Schema: public; Owner: user
--

ALTER SEQUENCE public.temporal_gap_audit_id_seq OWNED BY public.temporal_gap_audit.id;


--
-- Name: v_temporal_query_performance; Type: VIEW; Schema: public; Owner: user
--

CREATE VIEW public.v_temporal_query_performance AS
 SELECT 't'::text AS table_type,
    'organization_units'::text AS table_name,
...
ALTER VIEW public.v_temporal_query_performance OWNER TO "user";
...
--
-- Name: temporal_gap_audit id; Type: DEFAULT; Schema: public; Owner: user
--

ALTER TABLE ONLY public.temporal_gap_audit ALTER COLUMN id SET DEFAULT nextval('public.temporal_gap_audit_id_seq'::regclass);
...
--
-- Data for Name: organization_events; Type: TABLE DATA; Schema: public; Owner: user
--

COPY public.organization_events (event_id, organization_code, event_type, event_data, effective_date, end_date, created_by, created_at, tenant_id) FROM stdin;
ad3fc144-60bd-46cf-a060-bbefdb73b17d	1000001	UPDATE	{"name": "AI治理办公室-更新版"}	2025-12-01	\N	system	2025-08-10 05:51:57.611316+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
666b540f-1e9d-449d-b75c-ff387a3e439f	1000001	UPDATE	{"name": "深度测试更新"}	2025-12-01	\N	system	2025-08-10 06:03:40.695744+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
7666e486-3b6a-49ce-962a-6fcf5351a1b4	1000001	UPDATE	{"name": "并发测试-2-0"}	2025-12-25	\N	system	2025-08-10 06:06:41.951061+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
25141d5d-cf96-44c6-896b-83a7955008cf	1000001	UPDATE	{"name": "并发测试-1-0"}	2025-12-25	\N	system	2025-08-10 06:06:41.952142+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
fe220375-2f83-4cb0-b013-a42ae76df503	1000001	UPDATE	{"name": "并发测试-0-0"}	2025-12-25	\N	system	2025-08-10 06:06:41.956955+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
e7309dad-7925-4f83-a7da-32580559c613	1000001	UPDATE	{"name": "并发测试-2-1"}	2025-12-25	\N	system	2025-08-10 06:06:41.958915+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
9ca1859e-8dbd-411e-8f5e-30876aab306e	1000001	UPDATE	{"name": "并发测试-1-1"}	2025-12-25	\N	system	2025-08-10 06:06:41.961104+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
33543605-0ba4-4cf2-9176-c1d9befd2830	1000001	UPDATE	{"name": "并发测试-2-2"}	2025-12-25	\N	system	2025-08-10 06:06:41.964146+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
6de232e2-9691-4e95-9770-5e893aef4955	1000001	UPDATE	{"name": "并发测试-0-1"}	2025-12-25	\N	system	2025-08-10 06:06:41.965601+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
19c86e97-6bd8-47fc-bfa7-e2decc35628d	1000001	UPDATE	{"name": "并发测试-1-2"}	2025-12-25	\N	system	2025-08-10 06:06:41.967346+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
4fa2d64b-4d23-4469-b78e-92d8254fcf58	1000001	UPDATE	{"name": "并发测试-0-2"}	2025-12-25	\N	system	2025-08-10 06:06:41.970881+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
3b25bef4-d844-43ed-b3f5-01e93133fdbb	1000001	UPDATE	{"name": "混合负载测试0"}	2025-12-30	\N	system	2025-08-10 06:06:41.979447+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
34ce23d7-7a71-4edf-b8d8-3ad040e9cee6	1000001	UPDATE	{"name": "混合负载测试1"}	2025-12-30	\N	system	2025-08-10 06:06:42.186086+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
46fdfb7a-abce-43b3-972a-1e3888fd8a4c	1000001	UPDATE	{"name": "混合负载测试2"}	2025-12-30	\N	system	2025-08-10 06:06:42.394001+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
69ea65db-7275-4331-b1af-8afc711ecc9d	1000001	UPDATE	{"name": "混合负载测试3"}	2025-12-30	\N	system	2025-08-10 06:06:42.606531+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
e2fc0d0c-47cf-418c-b5c4-5a9d86535bd0	1000001	UPDATE	{"name": "混合负载测试4"}	2025-12-30	\N	system	2025-08-10 06:06:42.815098+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
d1c6cc45-4c99-4eb8-a02b-51de21e7e1d6	1000001	UPDATE	{"name": "顺序测试1"}	2025-11-01	\N	system	2025-08-10 06:06:43.053292+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
425058b4-3213-4fd8-ae31-9a8f43daf72f	1000001	UPDATE	{"name": "顺序测试2"}	2025-11-15	\N	system	2025-08-10 06:06:43.163664+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
07d0aebe-6790-4ce8-acff-fdb10151cfd4	1000001	UPDATE	{"name": "顺序测试3"}	2025-12-01	\N	system	2025-08-10 06:06:43.270839+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
577f079e-21ca-4fe4-84b7-eb577ba7173e	1000001	UPDATE	{"name": "CQRS集成测试更新"}	2025-12-20	\N	system	2025-08-10 06:09:40.619087+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
7853104b-4578-4bc2-9ca3-91895bec1e5b	1000001	UPDATE	{"name": "CQRS集成测试更新"}	2025-12-20	\N	system	2025-08-10 06:10:39.47616+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
aee2e20c-318b-448b-8c8b-ac066b02f86e	1000002	UPDATE	{"description": "部门职能调整，优化产品开发流程，提升团队协作效率"}	2025-03-01	\N	system	2025-08-10 23:30:43.562278+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
8982cde4-e19f-4133-8feb-d7c30402cc4b	1000002	UPDATE	{"name": "产品研发部", "description": "负责产品研发和技术创新"}	2025-04-01	\N	system	2025-08-10 23:39:50.295235+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
cf642db1-3751-492a-a69b-4711f139ac93	1000002	UPDATE	{"name": "产品部", "status": "ACTIVE", "unit_type": "DEPARTMENT", "sort_order": 0, "description": "产品管理部门"}	2025-08-11	\N	system	2025-08-11 00:21:32.839038+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
b8846cd2-3e17-4c44-a82b-1469066cc459	1000056	UPDATE	{"name": "测试部门_事件驱动更新", "description": "通过事件API更新的部门信息"}	2025-08-15	\N	system	2025-08-11 01:26:59.494721+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
7d284e66-1268-476d-b31c-7acc9e254a42	1000056	RESTRUCTURE	{"name": "重组后的测试部门", "status": "ACTIVE", "unit_type": "COST_CENTER"}	2025-08-12	\N	system	2025-08-11 01:31:45.030358+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
184703ec-1e39-46c5-8a43-5c929d349fea	1000056	DISSOLVE	{}	2025-12-31	\N	system	2025-08-11 01:32:53.157447+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
83348fa7-c81d-429c-8dda-64e0620f6be4	1000056	DEACTIVATE	{}	2025-09-01	\N	system	2025-08-11 01:33:50.863284+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
11640959-ef42-483a-b822-251af307668c	TEST001	UPDATE	{"description": "测试更新描述"}	2025-08-12	\N	system	2025-08-11 02:07:46.799843+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
5aefb154-03c4-4002-9cd7-d3b9c2b53328	TEST001	UPDATE	{"description": "测试更新描述"}	2025-08-12	\N	system	2025-08-11 02:17:47.552489+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
068858fe-4137-43a6-b658-9c2c8367b35a	1000056	RESTRUCTURE	{"name": "升级测试部门v5-时态管理测试", "description": "测试主从视图时态管理功能"}	2025-09-01	\N	system	2025-08-11 07:45:19.905107+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
7d1992ec-e47a-40c6-94a7-d44074b814e1	1000056	UPDATE	{"name": "升级测试部门v6-完整编辑功能测试", "description": "测试前端编辑表单集成功能"}	2025-10-01	\N	system	2025-08-11 07:49:09.696526+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
e485ed3b-1fe3-4df3-9368-2599b1365dd3	1000092	RESTRUCTURE	{"name": "时态管理测试组织v2-重组测试", "description": "前端编辑表单功能测试版本1"}	2025-09-01	\N	system	2025-08-11 07:49:45.709675+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
1ad39aa7-91dd-490d-a60e-873b7a7e3e24	1000000	UPDATE	{"name": "高谷集团(E2E测试版)", "status": "ACTIVE", "unit_type": "COMPANY", "description": "端到端测试创建的时态版本"}	2025-09-01	\N	system	2025-08-11 08:25:12.170545+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
3c006e72-6ac0-453a-bba7-b1896d4e1b79	1000000	UPDATE	{"name": "高谷集团(删除测试版)", "status": "ACTIVE", "unit_type": "COMPANY", "description": "用于测试删除功能的版本"}	2025-10-01	\N	system	2025-08-11 08:25:35.632769+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
57071aaa-9367-4252-a3ba-abada5985b65	1000081	UPDATE	{"name": "代理修复测试部门_时态更新成功", "description": "通过时态管理API进行的名称更新测试_最终验证"}	2025-08-11	\N	system	2025-08-11 11:51:45.515574+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
add4bde8-03eb-4eca-9c32-54aec6a4bcd3	1000081	UPDATE	{"status": "INACTIVE", "description": "状态变更测试 - 设置为非活跃"}	2025-08-12	\N	system	2025-08-11 12:05:15.045274+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
a01473bb-1d09-4cbc-90f9-bca417a708b3	1000081	RESTRUCTURE	{"name": "重组后的测试部门", "parent_code": "1000082"}	2025-08-13	\N	system	2025-08-11 12:06:09.568373+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
9407ddfd-349d-49bd-9da5-2fb350abdbc7	1000013	DISSOLVE	{}	2025-08-15	2025-08-16	system	2025-08-11 12:07:02.277179+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
f16e98d8-9513-4282-9d8f-8d4b61a3cc56	1000082	DEACTIVATE	{}	2025-08-14	\N	system	2025-08-11 12:07:24.202104+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
4eb87f36-efef-4c5b-933a-7eda5e240747	1000081	UPDATE	{"name": "性能测试1"}	2025-08-20	\N	system	2025-08-11 12:10:15.569162+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
e39e8dd6-7c5f-4205-b402-cdeef45790f1	1000056	UPDATE	{"name": "测试部门 - 时态管理功能验证v7", "description": "用于验证时态管理API的完整功能"}	2025-08-12	\N	system	2025-08-11 15:27:51.581762+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
57257dd5-860c-43a7-9df9-6720353b38d8	1000056	RESTRUCTURE	{"name": "重组后的时态管理测试部门v8", "unit_type": "COST_CENTER", "description": "组织架构重组后的新定位", "parent_code": "1000001"}	2025-09-01	\N	system	2025-08-11 15:28:57.330404+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
9c40153d-fdaf-477d-9d15-ff8b4c138e23	1000004	DISSOLVE	{}	2025-08-15	\N	system	2025-08-11 15:29:46.056774+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
1cfd7afd-e9da-4956-8943-99884df3f955	1000056	RESTRUCTURE	{"name": "全新重组测试部门v9", "unit_type": "COST_CENTER", "parent_code": ""}	2025-12-01	\N	system	2025-08-11 15:55:01.549001+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
b4212ecf-7809-4d72-9e00-a8c5cdbdd106	1000056	RESTRUCTURE	{"name": "时态管理测试部门v10_新增功能验证", "status": "PLANNED", "unit_type": "DEPARTMENT", "description": ""}	2026-01-01	\N	system	2025-08-11 17:00:58.236942+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
ccc4d0b6-f7c4-48be-a294-b3927ea9fdf1	1000056	RESTRUCTURE	{"name": "手工测试", "status": "PLANNED", "unit_type": "DEPARTMENT", "description": "启用"}	2026-02-12	\N	system	2025-08-11 17:05:45.532482+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
d4fd7b64-1523-4c77-a93f-015fef3c36df	1000056	RESTRUCTURE	{"name": "优化测试部门 - 集成表单版", "status": "PLANNED", "unit_type": "DEPARTMENT", "description": ""}	2026-03-01	\N	system	2025-08-11 17:19:05.972483+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
82fce00d-bb75-46c8-ae3d-7f53957e9ce2	1000056	RESTRUCTURE	{"name": "测试增加时态版本", "status": "PLANNED", "unit_type": "DEPARTMENT", "description": "测试2"}	2027-08-12	\N	system	2025-08-11 17:23:55.49076+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
6bc394dc-3ac0-4743-8296-e2c212da7e48	1000056	UPDATE	{"name": "更新后的测试部门2028", "description": "通过UPDATE事件更新的组织信息 - 2028年版本"}	2028-01-01	\N	system	2025-08-12 12:32:27.295248+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
d381d19c-79a6-4797-91c6-e49ed34d1c23	1000056	RESTRUCTURE	{"name": "重组后的创新实验室", "unit_type": "PROJECT_TEAM", "description": "通过RESTRUCTURE事件重组的项目团队", "parent_code": "1000057"}	2029-01-01	\N	system	2025-08-12 12:32:42.659027+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
d2462569-5f92-4ddb-9ce2-8de16e915efd	1000056	DISSOLVE	{"status": "INACTIVE", "description": "组织正式解散"}	2030-12-31	\N	system	2025-08-12 12:32:49.916533+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
6878d999-dec3-4bf6-bc3f-cf97608a8a03	1000056	UPDATE	{"name": "端到端测试更新组织2042", "description": "通过端到端测试创建的更新事件"}	2042-01-01	\N	system	2025-08-12 12:53:26.726283+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
927f068c-e63e-46ec-b25f-92f8289fcdd1	1000056	RESTRUCTURE	{"name": "端到端测试重组部门2043", "unit_type": "DEPARTMENT", "description": "通过端到端测试创建的重组事件"}	2043-01-01	\N	system	2025-08-12 12:53:26.754838+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
9251c4ee-dfd4-4fec-83da-79f316139706	1000056	RESTRUCTURE	{"name": "端到端测试未来重组部门v5", "description": "2044年生效的组织重组计划", "parent_code": "1000001"}	2044-01-01	\N	system	2025-08-12 21:55:23.879297+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
2e6b9beb-b1b8-4dcc-bc58-c5f116c4ebed	1000003	UPDATE	{"name": "销售总部", "status": "ACTIVE", "unit_type": "DEPARTMENT"}	2025-12-01	\N	system	2025-08-13 06:52:11.83201+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
a82edb66-b3fc-4509-ab37-80cbce9e9f96	1000003	UPDATE	{"name": "华东销售大区", "status": "ACTIVE", "unit_type": "DEPARTMENT"}	2026-03-01	\N	system	2025-08-13 06:52:20.15753+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
6a967159-e7bc-4a35-ae11-45d7f1af8b52	1000003	RESTRUCTURE	{"name": "全国销售总部", "status": "ACTIVE", "unit_type": "COMPANY"}	2026-07-01	\N	system	2025-08-13 06:52:27.421813+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
5f4652cf-cd28-43ef-94d6-a7cc87ca12a9	1000003	UPDATE	{"name": "全国销售总部", "status": "PLANNED", "unit_type": "COMPANY"}	2026-10-01	\N	system	2025-08-13 06:52:35.599499+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
342df265-079b-4daa-b3de-241c618baedb	1000003	UPDATE	{"name": "亚太销售总部", "status": "ACTIVE", "unit_type": "COMPANY"}	2027-01-01	\N	system	2025-08-13 06:52:44.313469+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
b70330ab-776a-49f1-85d8-bf733509c25d	1000003	UPDATE	{"name": "亚太销售总部", "status": "INACTIVE", "unit_type": "COMPANY"}	2027-06-01	\N	system	2025-08-13 06:52:52.583474+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
0de64b02-c2ee-4f39-ae2e-53dbe529a886	1000003	UPDATE	{"name": "全国销售总部", "status": "ACTIVE", "unit_type": "COMPANY"}	2026-01-01	\N	system	2025-08-13 07:13:12.58147+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
251ff21c-9862-4782-ba76-509f3ed0126c	1000003	RESTRUCTURE	{"name": "亚太销售总部", "status": "ACTIVE", "unit_type": "COMPANY"}	2027-01-01	\N	system	2025-08-13 07:13:21.33637+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
55a5ef8d-3807-479e-aa17-ec170edb243c	1000003	DEACTIVATE	null	2011-01-01	\N	system	2025-08-13 09:02:41.282216+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
73af8b77-0846-49c4-bc22-fb437f3bcc2f	1000003	DEACTIVATE	null	2011-01-01	\N	system	2025-08-13 09:14:34.278869+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
d58c3fcc-2d94-4076-855f-ded1083f013b	1000003	DEACTIVATE	null	2016-01-01	\N	system	2025-08-13 09:27:14.387533+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
f51e0047-8365-41e3-bbce-2c33d10f0147	1000003	DEACTIVATE	null	2006-01-01	\N	system	2025-08-13 10:40:23.564725+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
365956a5-10d5-4a94-8166-c6adb6cf949b	1000003	DEACTIVATE	null	2006-01-01	\N	system	2025-08-13 10:47:57.414016+00	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9
\.
...
COPY public.organization_units (code, parent_code, tenant_id, name, unit_type, status, level, path, sort_order, description, profile, created_at, updated_at, effective_date, end_date, change_reason, is_current, is_temporal) FROM stdin;
1001009	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC修复测试_1755066014	DEPARTMENT	ACTIVE	1	/1001009	0	测试修复后的CDC反序列化功能	{}	2025-08-13 06:20:14.989312+00	2025-08-13 06:20:14.990776+00	2025-08-13	\N	\N	t	f
1001010	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	市场推广组	DEPARTMENT	ACTIVE	2	/1000000/1001010	0		{}	2025-08-13 06:48:53.575331+00	2025-08-13 06:48:53.576229+00	2024-01-01	2024-02-29	\N	t	f
1001011	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	UUID测试部门	DEPARTMENT	ACTIVE	1	/1001011	0	测试UUID全局标识符功能	{}	2025-08-13 11:50:03.998271+00	2025-08-13 11:50:03.999446+00	2025-08-13	\N	\N	t	f
1001012	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	UUID测试部门2	DEPARTMENT	ACTIVE	1	/1001012	0	测试UUID实时同步功能	{}	2025-08-13 11:52:00.313285+00	2025-08-13 11:52:00.314483+00	2025-08-13	\N	\N	t	f
1001013	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	UUID同步验证部门	DEPARTMENT	ACTIVE	1	/1001013	0	验证UUID全局标识符和CDC同步功能	{}	2025-08-13 11:53:48.082526+00	2025-08-13 11:53:48.083488+00	2025-08-13	\N	\N	t	f
1001014	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	健康检查测试部门	DEPARTMENT	ACTIVE	1	/1001014	0		{}	2025-08-15 07:04:26.953601+00	2025-08-15 07:04:26.954916+00	2025-08-15	\N	\N	t	f
1001018	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	简化版测试部门	DEPARTMENT	ACTIVE	1	/1001018	0	测试简化版开发环境的创建功能	{}	2025-08-15 14:11:13.845035+00	2025-08-15 14:11:13.846276+00	2025-08-15	\N	\N	t	f
1000000	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	高谷集团	COMPANY	ACTIVE	1	/1000000	0	集团总部	{}	2025-08-13 02:55:20.05366+00	2025-08-13 02:55:20.05366+00	2025-08-13	\N	\N	t	f
1001019	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	诚实测试部门	DEPARTMENT	ACTIVE	1	/1001019	0	验证REST API创建功能的诚实测试	{}	2025-08-15 14:30:52.198971+00	2025-08-15 14:30:52.199966+00	2025-08-15	\N	\N	t	f
1000035	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门九	DEPARTMENT	INACTIVE	2	/1000000/1000035	0		{}	2025-08-08 06:06:12.680371+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000048	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十	DEPARTMENT	ACTIVE	2	/1000000/1000048	0		{}	2025-08-09 02:04:47.443405+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	全国销售总部	COMPANY	ACTIVE	2	/1000000/1000003	0	销售业务部门	{}	2025-08-13 07:13:12.58147+00	2025-08-13 08:53:49.511076+00	2026-01-01	2026-12-31	业务扩张升级为公司级别	f	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	亚太销售总部	COMPANY	ACTIVE	2	/1000000/1000003	0	销售业务部门	{}	2025-08-13 07:13:21.33637+00	2025-08-13 08:53:49.511076+00	2027-01-01	\N	全球化战略，成立亚太总部	f	f
1001015	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CPU测试部门	DEPARTMENT	ACTIVE	1	/1001015	0	用于测试CPU占用的部门	{}	2025-08-15 07:12:08.360389+00	2025-08-15 07:12:08.361499+00	2025-08-15	\N	\N	t	f
1001016	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	阶段一修复验证部门	DEPARTMENT	ACTIVE	1	/1001016	0	验证CPU修复后功能正常	{}	2025-08-15 10:08:55.610025+00	2025-08-15 10:08:55.612201+00	2025-08-15	\N	\N	t	f
1001017	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	健康检查测试部门	DEPARTMENT	ACTIVE	1	/1001017	0		{}	2025-08-15 10:51:22.385393+00	2025-08-15 10:51:22.389648+00	2025-08-15	\N	\N	t	f
1000057	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十一	DEPARTMENT	ACTIVE	2	/1000000/1000057	0		{}	2025-08-09 08:40:36.610696+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000061	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十二	DEPARTMENT	INACTIVE	2	/1000000/1000061	0		{}	2025-08-09 10:07:00.322212+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000037	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十	DEPARTMENT	INACTIVE	2	/1000000/1000037	0	测试新部门	{}	2025-08-08 07:04:27.723253+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000038	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十一	DEPARTMENT	INACTIVE	2	/1000000/1000038	0		{}	2025-08-08 07:08:40.505431+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000039	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十二	DEPARTMENT	INACTIVE	2	/1000000/1000039	0		{}	2025-08-08 08:04:49.327119+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	市场推广组	DEPARTMENT	ACTIVE	2	/1000000/1000003	0	销售业务部门	{"type": "sales"}	2025-08-05 11:23:01.426455+00	2025-08-13 08:53:49.511076+00	2000-01-01	2000-12-31	\N	f	f
1001007	1001006	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	研发部门	DEPARTMENT	ACTIVE	2	/1001006/1001007	10	产品研发部门	{}	2025-08-12 23:27:59.180956+00	2025-08-13 02:26:02.293402+00	2025-08-13	\N	\N	t	f
1001008	1001006	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	市场营销部	DEPARTMENT	ACTIVE	2	/1001006/1001008	20	市场营销和销售部门	{}	2025-08-12 23:28:07.973287+00	2025-08-13 02:26:02.293402+00	2025-08-13	\N	\N	t	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	销售中心	DEPARTMENT	INACTIVE	2	/1000000/1000003	0	销售业务部门	{}	2025-08-13 07:12:39.182004+00	2025-08-13 09:28:43.218135+00	2016-01-01	2025-12-31	\N	f	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	销售事业部	DEPARTMENT	ACTIVE	2	/1000000/1000003	0	销售业务部门	{}	2025-08-13 07:12:39.182004+00	2025-08-13 10:32:32.713272+00	2011-01-01	2025-12-31	\N	f	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	销售部门	DEPARTMENT	INACTIVE	2	/1000000/1000003	0	销售业务部门	{}	2025-08-13 07:12:39.182004+00	2025-08-13 10:47:57.414016+00	2006-01-01	2010-12-31	\N	f	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	销售团队	DEPARTMENT	ACTIVE	2	/1000000/1000003	0	销售业务部门	{}	2025-08-13 07:12:39.182004+00	2025-08-13 10:47:57.414016+00	2001-01-01	2010-12-31	\N	f	f
1000017	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	托尔斯泰2	DEPARTMENT	INACTIVE	2	/1000000/1000017	0		{}	2025-08-07 23:54:58.159621+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	销售部	DEPARTMENT	INACTIVE	2	/1000000/1000003	0	销售业务部门	{}	2025-08-13 07:12:55.238584+00	2025-08-13 10:40:23.564725+00	2025-01-01	2025-12-31	\N	t	f
1000028	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门四	COMPANY	INACTIVE	2	/1000000/1000028	0		{}	2025-08-08 05:34:03.038786+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000030	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门五	COMPANY	INACTIVE	2	/1000000/1000030	0		{}	2025-08-08 05:39:18.546437+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
1000031	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门6	DEPARTMENT	INACTIVE	2	/1000000/1000031	0		{}	2025-08-08 05:40:22.517792+00	2025-08-13 02:55:20.213188+00	2025-08-10	\N	\N	t	f
TEST001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	C部门	DEPARTMENT	ACTIVE	2	/1000000/TEST001	0	C部门	{}	2025-08-13 08:31:14.543558+00	2025-08-13 08:31:14.543558+00	2024-01-01	2025-12-31	\N	f	f
TEST001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	B部门	DEPARTMENT	INACTIVE	2	/1000000/TEST001	0	B部门（将被作废）	{}	2025-08-13 08:31:14.543558+00	2025-08-13 08:31:37.132665+00	2022-01-01	2023-12-31	\N	f	f
TEST001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	A部门	DEPARTMENT	ACTIVE	2	/1000000/TEST001	0	A部门	{}	2025-08-13 08:31:14.543558+00	2025-08-13 08:31:37.132665+00	2020-01-01	2023-12-31	\N	f	f
TEST003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	终结部门	DEPARTMENT	INACTIVE	2	/1000000/TEST003	0	终结部门（将被作废）	{}	2025-08-13 08:31:14.543558+00	2025-08-13 08:34:09.983586+00	2024-01-01	2025-12-31	\N	t	f
TEST003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	基础部门	DEPARTMENT	ACTIVE	2	/1000000/TEST003	0	基础部门	{}	2025-08-13 08:31:14.543558+00	2025-08-13 08:34:09.983586+00	2020-01-01	2025-12-31	\N	f	f
1000005	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	运维部	DEPARTMENT	ACTIVE	2	/1000000/1000005	0	测试修复后的CDC实时同步	{}	2025-08-06 06:43:14.71988+00	2025-08-13 02:26:02.293402+00	2025-08-10	\N	\N	t	f
TEST004	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	孤立部门	DEPARTMENT	INACTIVE	2	/1000000/TEST004	0	孤立部门（将被作废）	{}	2025-08-13 08:31:14.543558+00	2025-08-13 08:34:31.485822+00	2022-01-01	2023-12-31	\N	t	f
TEST002	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	后续部门	DEPARTMENT	ACTIVE	2	/1000000/TEST002	0	后续部门	{}	2025-08-13 08:33:30.59266+00	2025-08-13 08:33:30.59266+00	2020-01-01	2025-12-31	\N	t	f
TEST005	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	DELETE后	DEPARTMENT	ACTIVE	2	/1000000/TEST005	0	DELETE后	{}	2025-08-13 08:34:56.64059+00	2025-08-13 08:34:56.64059+00	2024-01-01	2025-12-31	\N	t	f
TEST005	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	DELETE前	DEPARTMENT	ACTIVE	2	/1000000/TEST005	0	DELETE前	{}	2025-08-13 08:34:56.64059+00	2025-08-13 08:34:56.64059+00	2020-01-01	2023-12-31	\N	f	f
PERF001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试C	DEPARTMENT	ACTIVE	2	/1000000/PERF001	0	性能测试	{}	2025-08-13 08:36:26.429902+00	2025-08-13 08:36:26.429902+00	2022-01-01	2022-12-31	\N	f	f
PERF001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试D	DEPARTMENT	ACTIVE	2	/1000000/PERF001	0	性能测试	{}	2025-08-13 08:36:26.429902+00	2025-08-13 08:36:26.429902+00	2023-01-01	2023-12-31	\N	t	f
PERF001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试B	DEPARTMENT	INACTIVE	2	/1000000/PERF001	0	性能测试	{}	2025-08-13 08:36:26.429902+00	2025-08-13 08:36:26.429902+00	2021-01-01	2021-12-31	\N	f	f
PERF001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试A	DEPARTMENT	ACTIVE	2	/1000000/PERF001	0	性能测试	{}	2025-08-13 08:36:26.429902+00	2025-08-13 08:36:26.429902+00	2020-01-01	2021-12-31	\N	f	f
1000004	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	人事部	DEPARTMENT	ACTIVE	2	/1000000/1000004	0	人力资源管理部门 - 重新激活	{}	2025-08-13 02:23:11.232965+00	2025-08-13 02:26:02.293402+00	2025-08-13	\N	修复缺失的当前记录	t	f
\.


--
-- Data for Name: organization_units_backup_pre_temporal; Type: TABLE DATA; Schema: public; Owner: user
--

COPY public.organization_units_backup_pre_temporal (code, parent_code, tenant_id, name, unit_type, status, level, path, sort_order, description, profile, created_at, updated_at) FROM stdin;
...
--
-- Data for Name: organization_units_backup_pre_version_removal; Type: TABLE DATA; Schema: public; Owner: user
--

COPY public.organization_units_backup_pre_version_removal (code, parent_code, tenant_id, name, unit_type, status, level, path, sort_order, description, profile, created_at, updated_at, effective_date, end_date, version, supersedes_version, change_reason, is_current, is_temporal) FROM stdin;
1000000	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	高谷集团	COMPANY	ACTIVE	1	/1000000	0	集团总公司	{"type": "headquarters"}	2025-08-05 11:23:01.426455+00	2025-08-05 11:23:01.426455+00	2025-08-10	\N	1	\N	\N	t	f
1000081	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	代理修复测试部门	DEPARTMENT	ACTIVE	1	/1000081	0	这是一个用于验证代理配置修复的测试部门	{}	2025-08-10 12:35:36.932884+00	2025-08-10 12:35:36.932884+00	2025-08-10	\N	1	\N	\N	t	f
1000082	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态管理修复测试部门	DEPARTMENT	ACTIVE	1	/1000082	0	验证时态管理无限循环修复是否成功	{}	2025-08-10 12:50:12.892984+00	2025-08-10 12:50:12.892984+00	2025-08-10	\N	1	\N	\N	t	f
1000013	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	前端修复测试组织	DEPARTMENT	INACTIVE	1	/1000013	0	测试前端修复是否生效	{}	2025-08-07 12:59:43.272937+00	2025-08-08 08:29:48.077206+00	2025-08-10	\N	1	\N	\N	t	f
1000015	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	修复后测试部门	DEPARTMENT	INACTIVE	1	/1000015	0		{}	2025-08-07 13:13:16.726148+00	2025-08-08 08:29:50.749104+00	2025-08-10	\N	1	\N	\N	t	f
1000038	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十一	DEPARTMENT	INACTIVE	1	/1000038	0		{}	2025-08-08 07:08:40.505431+00	2025-08-08 08:29:53.348862+00	2025-08-10	\N	1	\N	\N	t	f
1000039	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十二	DEPARTMENT	INACTIVE	1	/1000039	0		{}	2025-08-08 08:04:49.327119+00	2025-08-08 08:29:55.720836+00	2025-08-10	\N	1	\N	\N	t	f
CUSTOM-001	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门自定义编码	DEPARTMENT	INACTIVE	1	/CUSTOM-001	0	测试用户自定义编码功能	{}	2025-08-08 05:54:01.321143+00	2025-08-08 08:29:59.435129+00	2025-08-10	\N	1	\N	\N	t	f
1000037	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十	DEPARTMENT	INACTIVE	1	/1000037	0	测试新部门	{}	2025-08-08 07:04:27.723253+00	2025-08-08 08:30:02.006375+00	2025-08-10	\N	1	\N	\N	t	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	销售部	DEPARTMENT	ACTIVE	2	/1000000/1000003	0	销售业务部门	{"type": "sales"}	2025-08-05 11:23:01.426455+00	2025-08-06 06:13:47.072807+00	2025-08-10	\N	1	\N	\N	t	f
1000090	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	2026年新部门	DEPARTMENT	PLANNED	1	/1000090	0	计划在2026年启动的新部门	{}	2025-08-10 14:29:12.554038+00	2025-08-10 14:29:12.555262+00	2026-01-01	2026-12-31	1	\N	业务扩展需要	t	t
1000032	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门自动编码	DEPARTMENT	INACTIVE	1	/1000032	0		{}	2025-08-08 05:53:51.209306+00	2025-08-08 08:30:04.197299+00	2025-08-10	\N	1	\N	\N	t	f
1000031	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门6	DEPARTMENT	INACTIVE	1	/1000031	0		{}	2025-08-08 05:40:22.517792+00	2025-08-08 08:30:06.055407+00	2025-08-10	\N	1	\N	\N	t	f
1000008	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000008	0	性能测试创建的部门	{}	2025-08-06 08:20:40.514638+00	2025-08-06 08:20:58.686919+00	2025-08-10	\N	1	\N	\N	t	f
1000030	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门五	COMPANY	INACTIVE	1	/1000030	0		{}	2025-08-08 05:39:18.546437+00	2025-08-08 08:30:10.056275+00	2025-08-10	\N	1	\N	\N	t	f
1000029	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	最终验收测试部门	DEPARTMENT	INACTIVE	1	/1000029	0	验证表单重复提交修复的最终测试	{}	2025-08-08 05:34:37.145632+00	2025-08-08 08:30:12.169852+00	2025-08-10	\N	1	\N	\N	t	f
1000012	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试组织创建	COMPANY	INACTIVE	1	/1000012	0	API测试	{}	2025-08-07 12:54:53.171785+00	2025-08-08 08:30:16.049528+00	2025-08-10	\N	1	\N	\N	t	f
1000014	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	最终测试验证组织	DEPARTMENT	INACTIVE	1	/1000014	0	确认端口修复成功	{}	2025-08-07 13:07:58.098469+00	2025-08-08 08:30:18.151684+00	2025-08-10	\N	1	\N	\N	t	f
1000022	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	页面测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000022	0		{}	2025-08-08 04:28:16.951531+00	2025-08-08 08:30:20.610268+00	2025-08-10	\N	1	\N	\N	t	f
1000011	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	实时监控测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000011	0	实时监控同步的测试部门	{}	2025-08-06 09:14:34.213943+00	2025-08-08 02:37:46.497481+00	2025-08-10	\N	1	\N	\N	t	f
1000018	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	Canvas Kit Modal测试部门	DEPARTMENT	ACTIVE	1	/1000018	0	验证Canvas Kit v13 Modal API修复后的端到端功能	{}	2025-08-08 03:46:21.395223+00	2025-08-08 03:46:21.39713+00	2025-08-10	\N	1	\N	\N	t	f
1000026	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	重复提交测试部门	DEPARTMENT	INACTIVE	1	/1000026	0		{}	2025-08-08 05:04:48.899194+00	2025-08-08 08:30:43.63678+00	2025-08-10	\N	1	\N	\N	t	f
1000016	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CQRS测试部门	DEPARTMENT	INACTIVE	1	/1000016	0	用于验证CQRS命令服务功能	{}	2025-08-07 22:53:56.116101+00	2025-08-08 04:06:33.426054+00	2025-08-10	\N	1	\N	\N	t	f
1000027	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	修复验证测试部门-已更新	DEPARTMENT	INACTIVE	1	/1000027	0	更新测试成功	{}	2025-08-08 05:18:23.200839+00	2025-08-08 08:30:47.876905+00	2025-08-10	\N	1	\N	\N	t	f
1000019	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门	DEPARTMENT	ACTIVE	1	/1000019	0	\N	{}	2025-08-08 03:54:57.175359+00	2025-08-08 03:54:57.175716+00	2025-08-10	\N	1	\N	\N	t	f
1000020	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	API修复测试部门	DEPARTMENT	ACTIVE	2	/1000000/1000020	0		{}	2025-08-08 04:00:32.996385+00	2025-08-08 04:00:32.996721+00	2025-08-10	\N	1	\N	\N	t	f
1000028	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门四	COMPANY	INACTIVE	1	/1000028	0		{}	2025-08-08 05:34:03.038786+00	2025-08-08 08:30:48.424042+00	2025-08-10	\N	1	\N	\N	t	f
1000021	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	E2E测试部门_更新版	DEPARTMENT	INACTIVE	2	/1000000/1000021	0	\N	{}	2025-08-08 04:21:36.617388+00	2025-08-08 04:22:24.960447+00	2025-08-10	\N	1	\N	\N	t	f
1000002	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	产品部	DEPARTMENT	ACTIVE	2	/1000000/1000002	0	产品管理部门 - 已更新测试	{"type": "product"}	2025-08-05 11:23:01.426455+00	2025-08-11 00:21:32.839038+00	2025-08-05	2025-08-10	1	\N	数据重置	f	f
1000017	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	托尔斯泰2	DEPARTMENT	INACTIVE	1	/1000017	0		{}	2025-08-07 23:54:58.159621+00	2025-08-08 04:55:32.135861+00	2025-08-10	\N	1	\N	\N	t	f
1000024	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	验收测试部门_已编辑	DEPARTMENT	INACTIVE	1	/1000024	0		{}	2025-08-08 05:03:04.121248+00	2025-08-08 23:35:31.076786+00	2025-08-10	\N	1	\N	\N	t	f
1000025	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	错误测试部门	DEPARTMENT	INACTIVE	1	/1000025	0		{}	2025-08-08 05:03:45.715432+00	2025-08-08 23:36:05.754032+00	2025-08-10	\N	1	\N	\N	t	f
1000023	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	修复成功测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000023	0		{}	2025-08-08 04:53:42.734942+00	2025-08-09 00:38:10.615696+00	2025-08-10	\N	1	\N	\N	t	f
1000035	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门九	DEPARTMENT	INACTIVE	1	/1000035	0		{}	2025-08-08 06:06:12.680371+00	2025-08-08 06:10:57.246611+00	2025-08-10	\N	1	\N	\N	t	f
1000034	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试自动编码部门	DEPARTMENT	INACTIVE	1	/1000034	0		{}	2025-08-08 06:05:15.725628+00	2025-08-08 06:11:17.502145+00	2025-08-10	\N	1	\N	\N	t	f
1000033	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	页面测试部门2	DEPARTMENT	INACTIVE	1	/1000033	0	测试页面编码自动生成功能	{}	2025-08-08 06:04:58.171327+00	2025-08-08 06:11:53.307689+00	2025-08-10	\N	1	\N	\N	t	f
1000036	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	状态更新测试部门	DEPARTMENT	INACTIVE	1	/1000036	0		{}	2025-08-08 06:15:42.18495+00	2025-08-08 06:16:32.022373+00	2025-08-10	\N	1	\N	\N	t	f
1000010	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	数据同步测试部门_FINAL	DEPARTMENT	ACTIVE	2	/1000000/1000010	0	用于验证数据同步的测试部门	{}	2025-08-06 09:11:59.778608+00	2025-08-09 05:41:18.00958+00	2025-08-10	\N	1	\N	\N	t	f
1000009	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	GraphQL端到端测试部门	DEPARTMENT	ACTIVE	2	/1000000/1000009	0	用于验证GraphQL集成的测试部门	{}	2025-08-06 08:53:30.92514+00	2025-08-09 07:45:01.087376+00	2025-08-10	\N	1	\N	\N	t	f
1000005	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	运维部	DEPARTMENT	ACTIVE	2	/1000000/1000005	0	测试修复后的CDC实时同步	{}	2025-08-06 06:43:14.71988+00	2025-08-09 10:42:21.152661+00	2025-08-10	\N	1	\N	\N	t	f
1000001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	AI治理办公室	DEPARTMENT	ACTIVE	2	/1000000/1000001	0	技术研发部门	{"type": "rd"}	2025-08-05 11:23:01.426455+00	2025-08-09 12:07:15.838099+00	2025-08-10	\N	1	\N	\N	t	f
1000004	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	人事部	DEPARTMENT	ACTIVE	2	/1000000/1000004	0	人力资源部门	{"type": "hr"}	2025-08-05 11:23:01.426455+00	2025-08-10 03:15:49.936921+00	2025-08-10	\N	1	\N	\N	t	f
1000007	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CoreHR测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000007	0	修改后的部门描述 - MCP验证测试	{}	2025-08-06 07:09:08.005666+00	2025-08-10 02:49:58.073305+00	2025-08-10	\N	1	\N	\N	t	f
1000006	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	同步测试部门禁用-公司-3级-描述4-排序7	COMPANY	INACTIVE	2	/1000000/1000006	7	描述4	{}	2025-08-06 07:00:33.177944+00	2025-08-10 03:55:14.63456+00	2025-08-10	\N	1	\N	\N	t	f
1000040	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754668553755	DEPARTMENT	ACTIVE	1	/1000040	0	这是一个测试部门	{}	2025-08-08 15:55:53.759589+00	2025-08-08 15:55:53.760634+00	2025-08-10	\N	1	\N	\N	t	f
1000041	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754669118696	DEPARTMENT	ACTIVE	1	/1000041	0	这是一个测试部门	{}	2025-08-08 16:05:18.711949+00	2025-08-08 16:05:18.712276+00	2025-08-10	\N	1	\N	\N	t	f
1000042	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754669124970	DEPARTMENT	ACTIVE	1	/1000042	0	这是一个测试部门	{}	2025-08-08 16:05:24.97627+00	2025-08-08 16:05:24.976671+00	2025-08-10	\N	1	\N	\N	t	f
1000043	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754669129397	DEPARTMENT	ACTIVE	1	/1000043	0	这是一个测试部门	{}	2025-08-08 16:05:29.412342+00	2025-08-08 16:05:29.412733+00	2025-08-10	\N	1	\N	\N	t	f
1000044	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754669139984	DEPARTMENT	ACTIVE	1	/1000044	0	这是一个测试部门	{}	2025-08-08 16:05:40.001238+00	2025-08-08 16:05:40.001654+00	2025-08-10	\N	1	\N	\N	t	f
1000045	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	全面测试部门	DEPARTMENT	ACTIVE	1	/1000045	0		{}	2025-08-08 23:33:02.396731+00	2025-08-08 23:33:02.398649+00	2025-08-10	\N	1	\N	\N	t	f
1000046	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754703523513	DEPARTMENT	ACTIVE	1	/1000046	0	这是一个测试部门	{}	2025-08-09 01:38:43.274837+00	2025-08-09 01:38:43.276731+00	2025-08-10	\N	1	\N	\N	t	f
1000047	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754703527894	DEPARTMENT	ACTIVE	1	/1000047	0	这是一个测试部门	{}	2025-08-09 01:38:47.922057+00	2025-08-09 01:38:47.922372+00	2025-08-10	\N	1	\N	\N	t	f
1000048	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十	DEPARTMENT	ACTIVE	1	/1000048	0		{}	2025-08-09 02:04:47.443405+00	2025-08-09 02:04:47.443898+00	2025-08-10	\N	1	\N	\N	t	f
1000083	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态管理测试部门(升级版)	DEPARTMENT	ACTIVE	1	/1000083	0		{}	2025-08-10 12:58:56.152848+00	2025-08-10 12:59:54.466027+00	2025-08-20	2025-12-31	1	\N	测试版本更新功能	t	t
1000049	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门0	DEPARTMENT	ACTIVE	1	/1000049	0	\N	{}	2025-08-09 03:01:03.157302+00	2025-08-09 03:01:03.158256+00	2025-08-10	\N	1	\N	\N	t	f
1000050	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门1	DEPARTMENT	ACTIVE	1	/1000050	0	\N	{}	2025-08-09 03:01:03.324159+00	2025-08-09 03:01:03.324592+00	2025-08-10	\N	1	\N	\N	t	f
1000051	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门2	DEPARTMENT	ACTIVE	1	/1000051	0	\N	{}	2025-08-09 03:01:03.451726+00	2025-08-09 03:01:03.452036+00	2025-08-10	\N	1	\N	\N	t	f
1000052	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门3	DEPARTMENT	ACTIVE	1	/1000052	0	\N	{}	2025-08-09 03:01:03.576029+00	2025-08-09 03:01:03.576217+00	2025-08-10	\N	1	\N	\N	t	f
1000089	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态测试组织-更新	DEPARTMENT	ACTIVE	1	/1000089	0	测试时态字段更新	{}	2025-08-10 14:26:47.809083+00	2025-08-10 14:29:52.26065+00	2024-06-01	2024-11-30	1	\N	调整生效期间	t	f
1000054	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	Phase2测试部门	DEPARTMENT	ACTIVE	1	/1000054	0	验证简化测试	{}	2025-08-09 05:05:54.217338+00	2025-08-09 05:05:54.218356+00	2025-08-10	\N	1	\N	\N	t	f
1000055	1000054	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	DDD简化更新测试	DEPARTMENT	INACTIVE	2	/1000054/1000055	0	测试层次计算	{}	2025-08-09 05:11:37.2748+00	2025-08-09 05:12:13.111427+00	2025-08-10	\N	1	\N	\N	t	f
C722694	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC端到端测试	DEPARTMENT	ACTIVE	1	/C722694	1	验证CDC功能	{}	2025-08-09 06:58:14.346048+00	2025-08-09 06:58:14.346048+00	2025-08-10	\N	1	\N	\N	t	f
S722950	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	数据同步测试	DEPARTMENT	ACTIVE	1	/S722950	1	验证Neo4j同步功能	{}	2025-08-09 07:02:30.110333+00	2025-08-09 07:02:30.110333+00	2025-08-10	\N	1	\N	\N	t	f
U723048	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	解析修复测试	DEPARTMENT	ACTIVE	1	/U723048	1	验证unwrapped CDC事件	{}	2025-08-09 07:04:08.52621+00	2025-08-09 07:04:08.52621+00	2025-08-10	\N	1	\N	\N	t	f
F723128	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC修复验证	DEPARTMENT	ACTIVE	1	/F723128	1	验证schema消息解析	{}	2025-08-09 07:05:28.554088+00	2025-08-09 07:05:28.554088+00	2025-08-10	\N	1	\N	\N	t	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试更新缓存_同步修复	DEPARTMENT	ACTIVE	1	/1000056	0	测试同步服务修复后的缓存失效	{}	2025-08-09 07:21:10.177689+00	2025-08-09 08:13:00.514444+00	2025-08-10	\N	1	\N	\N	t	f
1000053	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门4	DEPARTMENT	INACTIVE	1	/1000053	0	\N	{}	2025-08-09 03:01:03.703919+00	2025-08-09 08:16:23.560124+00	2025-08-10	\N	1	\N	\N	t	f
1000058	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	同步测试部门_验证	DEPARTMENT	ACTIVE	1	/1000058	0		{}	2025-08-09 08:47:46.220656+00	2025-08-09 08:47:46.221894+00	2025-08-10	\N	1	\N	\N	t	f
1000059	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	重新同步测试_部门	DEPARTMENT	ACTIVE	1	/1000059	0		{}	2025-08-09 08:49:46.600642+00	2025-08-09 08:49:46.602372+00	2025-08-10	\N	1	\N	\N	t	f
1000057	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十一	DEPARTMENT	ACTIVE	1	/1000057	0		{}	2025-08-09 08:40:36.610696+00	2025-08-09 08:52:50.589984+00	2025-08-10	\N	1	\N	\N	t	f
1000060	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	前端修复验证部门	DEPARTMENT	ACTIVE	1	/1000060	0	验证前端修复是否成功的测试部门	{}	2025-08-09 09:59:25.060107+00	2025-08-09 09:59:25.063766+00	2025-08-10	\N	1	\N	\N	t	f
1000061	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十二	DEPARTMENT	INACTIVE	1	/1000061	0		{}	2025-08-09 10:07:00.322212+00	2025-08-09 10:07:21.47672+00	2025-08-10	\N	1	\N	\N	t	f
1000062	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	E2E测试部门_缓存失效测试	DEPARTMENT	ACTIVE	1	/1000062	0		{}	2025-08-09 10:50:35.015368+00	2025-08-09 10:53:17.206794+00	2025-08-10	\N	1	\N	\N	t	f
1000063	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试组织_1754736868	DEPARTMENT	ACTIVE	1	/1000063	0		{}	2025-08-09 10:54:28.700026+00	2025-08-09 10:54:28.70107+00	2025-08-10	\N	1	\N	\N	t	f
1000064	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试组织(已更新)	DEPARTMENT	ACTIVE	2	/1000000/1000064	0		{}	2025-08-09 12:09:43.433069+00	2025-08-09 12:09:50.151959+00	2025-08-10	\N	1	\N	\N	t	f
1000065	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	端到端测试部门_已更新	DEPARTMENT	INACTIVE	2	/1000000/1000065	0		{}	2025-08-09 12:24:34.885664+00	2025-08-09 12:30:03.646997+00	2025-08-10	\N	1	\N	\N	t	f
1000066	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	健康检查测试部门	DEPARTMENT	ACTIVE	1	/1000066	0		{}	2025-08-09 12:30:31.109551+00	2025-08-09 12:30:31.111004+00	2025-08-10	\N	1	\N	\N	t	f
1000067	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	删除测试部门_1754742927	DEPARTMENT	INACTIVE	1	/1000067	0	用于测试删除功能	{}	2025-08-09 12:35:27.966243+00	2025-08-09 12:35:27.983511+00	2025-08-10	\N	1	\N	\N	t	f
1000068	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC测试部门_1754742983	DEPARTMENT	ACTIVE	1	/1000068	0	CDC同步测试	{}	2025-08-09 12:36:28.515331+00	2025-08-09 12:36:28.516233+00	2025-08-10	\N	1	\N	\N	t	f
1000069	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	缓存失效测试_1754743228	DEPARTMENT	ACTIVE	1	/1000069	0	测试缓存失效机制	{}	2025-08-09 12:40:35.286829+00	2025-08-09 12:40:35.288004+00	2025-08-10	\N	1	\N	\N	t	f
1000070	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	缓存失效测试2_1754743246	DEPARTMENT	ACTIVE	1	/1000070	0	再次测试缓存失效	{}	2025-08-09 12:40:46.337728+00	2025-08-09 12:40:46.338665+00	2025-08-10	\N	1	\N	\N	t	f
1000071	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_1	DEPARTMENT	ACTIVE	1	/1000071	0	性能压力测试	{}	2025-08-09 12:53:08.788862+00	2025-08-09 12:53:08.789868+00	2025-08-10	\N	1	\N	\N	t	f
1000072	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_2	DEPARTMENT	ACTIVE	1	/1000072	0	性能压力测试	{}	2025-08-09 12:53:08.810222+00	2025-08-09 12:53:08.811267+00	2025-08-10	\N	1	\N	\N	t	f
1000073	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_3	DEPARTMENT	ACTIVE	1	/1000073	0	性能压力测试	{}	2025-08-09 12:53:08.825371+00	2025-08-09 12:53:08.826261+00	2025-08-10	\N	1	\N	\N	t	f
1000074	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_4	DEPARTMENT	ACTIVE	1	/1000074	0	性能压力测试	{}	2025-08-09 12:53:08.837663+00	2025-08-09 12:53:08.838478+00	2025-08-10	\N	1	\N	\N	t	f
1000075	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_5	DEPARTMENT	ACTIVE	1	/1000075	0	性能压力测试	{}	2025-08-09 12:53:08.850776+00	2025-08-09 12:53:08.851811+00	2025-08-10	\N	1	\N	\N	t	f
1000084	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态测试部门	DEPARTMENT	ACTIVE	1	/1000084	10	测试时态字段修复	{}	2025-08-10 13:35:20.789341+00	2025-08-10 13:35:20.7903+00	2024-08-10	2024-12-31	1	\N	测试日期类型修复	t	t
1000085	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态测试部门2	DEPARTMENT	ACTIVE	1	/1000085	20	测试修复后的时态字段同步	{}	2025-08-10 13:42:51.280224+00	2025-08-10 13:42:51.281228+00	2024-08-11	2024-12-30	1	\N	测试字段修复后的同步	t	t
1000086	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态同步测试3	DEPARTMENT	ACTIVE	1	/1000086	30	测试修复后的DebeziumDate处理	{}	2025-08-10 13:47:55.215923+00	2025-08-10 13:47:55.217765+00	2024-08-12	2024-12-29	1	\N	测试DebeziumDate修复	t	t
1000076	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试编辑部门	COST_CENTER	ACTIVE	1	/1000076	0	完整编辑模式测试 - 所有字段可编辑	{}	2025-08-10 03:02:51.529857+00	2025-08-10 03:15:03.677593+00	2025-08-10	\N	1	\N	\N	t	f
1000087	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	最终测试部门	DEPARTMENT	ACTIVE	1	/1000087	40	最终验证修复	{}	2025-08-10 13:51:06.866115+00	2025-08-10 13:51:06.867187+00	2024-08-13	2024-12-28	1	\N	最终DebeziumDate测试	t	t
1000088	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	调试测试部门	DEPARTMENT	ACTIVE	1	/1000088	50	调试RawMessage处理	{}	2025-08-10 13:53:03.469378+00	2025-08-10 13:53:03.470626+00	2024-08-14	2024-12-27	1	\N	RawMessage调试测试	t	t
1000077	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	生产环境测试部门	DEPARTMENT	ACTIVE	2	/1000000/1000077	0	生产环境部署验证测试	{}	2025-08-10 06:27:01.951103+00	2025-08-10 06:27:01.952358+00	2025-08-10	\N	1	\N	\N	t	f
1000080	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC同步测试部门	DEPARTMENT	ACTIVE	1	/1000080	0	测试CDC数据同步功能	{}	2025-08-10 12:02:56.262746+00	2025-08-10 12:02:56.262746+00	2025-08-10	\N	1	\N	\N	t	f
\.


--
-- Data for Name: organization_units_backup_temporal; Type: TABLE DATA; Schema: public; Owner: user
--

COPY public.organization_units_backup_temporal (code, parent_code, tenant_id, name, unit_type, status, level, path, sort_order, description, profile, created_at, updated_at, effective_date, end_date, change_reason, is_current, is_temporal) FROM stdin;
1000999	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试时态组织V1	DEPARTMENT	ACTIVE	1	/1000999	0	\N	{}	2025-08-11 11:54:24.112001+00	2025-08-11 11:54:24.112001+00	2025-08-01	2025-08-04	\N	f	f
1000999	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试时态组织V2	DEPARTMENT	ACTIVE	1	/1000999	0	\N	{}	2025-08-11 11:54:24.112001+00	2025-08-11 11:54:24.112001+00	2025-08-05	2025-08-09	\N	f	f
1000999	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试时态组织V3	DEPARTMENT	ACTIVE	1	/1000999	0	\N	{}	2025-08-11 11:54:24.112001+00	2025-08-11 11:54:24.112001+00	2025-08-10	\N	\N	t	f
1000015	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	修复后测试部门	DEPARTMENT	INACTIVE	1	/1000015	0		{}	2025-08-07 13:13:16.726148+00	2025-08-08 08:29:50.749104+00	2025-08-10	\N	\N	t	f
1000038	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十一	DEPARTMENT	INACTIVE	1	/1000038	0		{}	2025-08-08 07:08:40.505431+00	2025-08-08 08:29:53.348862+00	2025-08-10	\N	\N	t	f
1000039	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十二	DEPARTMENT	INACTIVE	1	/1000039	0		{}	2025-08-08 08:04:49.327119+00	2025-08-08 08:29:55.720836+00	2025-08-10	\N	\N	t	f
CUSTOM-001	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门自定义编码	DEPARTMENT	INACTIVE	1	/CUSTOM-001	0	测试用户自定义编码功能	{}	2025-08-08 05:54:01.321143+00	2025-08-08 08:29:59.435129+00	2025-08-10	\N	\N	t	f
1000037	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门十	DEPARTMENT	INACTIVE	1	/1000037	0	测试新部门	{}	2025-08-08 07:04:27.723253+00	2025-08-08 08:30:02.006375+00	2025-08-10	\N	\N	t	f
1000082	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态管理修复测试部门	DEPARTMENT	INACTIVE	1	/1000082	0	验证时态管理无限循环修复是否成功	{}	2025-08-10 12:50:12.892984+00	2025-08-11 12:07:24.202104+00	2025-08-10	\N	\N	t	f
1000003	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	销售部	DEPARTMENT	ACTIVE	2	/1000000/1000003	0	销售业务部门	{"type": "sales"}	2025-08-05 11:23:01.426455+00	2025-08-06 06:13:47.072807+00	2025-08-10	\N	\N	t	f
1000090	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	2026年新部门	DEPARTMENT	PLANNED	1	/1000090	0	计划在2026年启动的新部门	{}	2025-08-10 14:29:12.554038+00	2025-08-10 14:29:12.555262+00	2026-01-01	2026-12-31	业务扩展需要	t	t
1000032	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门自动编码	DEPARTMENT	INACTIVE	1	/1000032	0		{}	2025-08-08 05:53:51.209306+00	2025-08-08 08:30:04.197299+00	2025-08-10	\N	\N	t	f
1000031	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门6	DEPARTMENT	INACTIVE	1	/1000031	0		{}	2025-08-08 05:40:22.517792+00	2025-08-08 08:30:06.055407+00	2025-08-10	\N	\N	t	f
1000008	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000008	0	性能测试创建的部门	{}	2025-08-06 08:20:40.514638+00	2025-08-06 08:20:58.686919+00	2025-08-10	\N	\N	t	f
1000030	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门五	COMPANY	INACTIVE	1	/1000030	0		{}	2025-08-08 05:39:18.546437+00	2025-08-08 08:30:10.056275+00	2025-08-10	\N	\N	t	f
1000029	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	最终验收测试部门	DEPARTMENT	INACTIVE	1	/1000029	0	验证表单重复提交修复的最终测试	{}	2025-08-08 05:34:37.145632+00	2025-08-08 08:30:12.169852+00	2025-08-10	\N	\N	t	f
1000012	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试组织创建	COMPANY	INACTIVE	1	/1000012	0	API测试	{}	2025-08-07 12:54:53.171785+00	2025-08-08 08:30:16.049528+00	2025-08-10	\N	\N	t	f
1000014	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	最终测试验证组织	DEPARTMENT	INACTIVE	1	/1000014	0	确认端口修复成功	{}	2025-08-07 13:07:58.098469+00	2025-08-08 08:30:18.151684+00	2025-08-10	\N	\N	t	f
1000004	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	人事部	DEPARTMENT	INACTIVE	2	/1000000/1000004	0	人力资源部门	{"type": "hr"}	2025-08-05 11:23:01.426455+00	2025-08-11 15:29:46.056774+00	2025-08-10	2025-08-15	\N	f	f
1000022	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	页面测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000022	0		{}	2025-08-08 04:28:16.951531+00	2025-08-08 08:30:20.610268+00	2025-08-10	\N	\N	t	f
1000011	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	实时监控测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000011	0	实时监控同步的测试部门	{}	2025-08-06 09:14:34.213943+00	2025-08-08 02:37:46.497481+00	2025-08-10	\N	\N	t	f
1000018	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	Canvas Kit Modal测试部门	DEPARTMENT	ACTIVE	1	/1000018	0	验证Canvas Kit v13 Modal API修复后的端到端功能	{}	2025-08-08 03:46:21.395223+00	2025-08-08 03:46:21.39713+00	2025-08-10	\N	\N	t	f
1000026	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	重复提交测试部门	DEPARTMENT	INACTIVE	1	/1000026	0		{}	2025-08-08 05:04:48.899194+00	2025-08-08 08:30:43.63678+00	2025-08-10	\N	\N	t	f
1000016	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CQRS测试部门	DEPARTMENT	INACTIVE	1	/1000016	0	用于验证CQRS命令服务功能	{}	2025-08-07 22:53:56.116101+00	2025-08-08 04:06:33.426054+00	2025-08-10	\N	\N	t	f
1000027	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	修复验证测试部门-已更新	DEPARTMENT	INACTIVE	1	/1000027	0	更新测试成功	{}	2025-08-08 05:18:23.200839+00	2025-08-08 08:30:47.876905+00	2025-08-10	\N	\N	t	f
1000019	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门	DEPARTMENT	ACTIVE	1	/1000019	0	\N	{}	2025-08-08 03:54:57.175359+00	2025-08-08 03:54:57.175716+00	2025-08-10	\N	\N	t	f
1000020	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	API修复测试部门	DEPARTMENT	ACTIVE	2	/1000000/1000020	0		{}	2025-08-08 04:00:32.996385+00	2025-08-08 04:00:32.996721+00	2025-08-10	\N	\N	t	f
1000028	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门四	COMPANY	INACTIVE	1	/1000028	0		{}	2025-08-08 05:34:03.038786+00	2025-08-08 08:30:48.424042+00	2025-08-10	\N	\N	t	f
1000021	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	E2E测试部门_更新版	DEPARTMENT	INACTIVE	2	/1000000/1000021	0	\N	{}	2025-08-08 04:21:36.617388+00	2025-08-08 04:22:24.960447+00	2025-08-10	\N	\N	t	f
1000002	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	产品部	DEPARTMENT	ACTIVE	2	/1000000/1000002	0	产品管理部门 - 已更新测试	{"type": "product"}	2025-08-05 11:23:01.426455+00	2025-08-11 00:21:32.839038+00	2025-08-05	2025-08-10	数据重置	f	f
1000017	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	托尔斯泰2	DEPARTMENT	INACTIVE	1	/1000017	0		{}	2025-08-07 23:54:58.159621+00	2025-08-08 04:55:32.135861+00	2025-08-10	\N	\N	t	f
1000007	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CoreHR测试部门_MCP编辑测试	DEPARTMENT	INACTIVE	2	/1000000/1000007	0	MCP浏览器测试 - 编辑功能验证 - 更新时间：2025-08-11	{}	2025-08-06 07:09:08.005666+00	2025-08-11 05:20:03.626347+00	2025-08-10	\N	\N	t	f
1000024	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	验收测试部门_已编辑	DEPARTMENT	INACTIVE	1	/1000024	0		{}	2025-08-08 05:03:04.121248+00	2025-08-08 23:35:31.076786+00	2025-08-10	\N	\N	t	f
1000025	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	错误测试部门	DEPARTMENT	INACTIVE	1	/1000025	0		{}	2025-08-08 05:03:45.715432+00	2025-08-08 23:36:05.754032+00	2025-08-10	\N	\N	t	f
1000023	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	修复成功测试部门	DEPARTMENT	INACTIVE	2	/1000000/1000023	0		{}	2025-08-08 04:53:42.734942+00	2025-08-09 00:38:10.615696+00	2025-08-10	\N	\N	t	f
1000081	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	代理修复测试部门	DEPARTMENT	ACTIVE	1	/1000081	0	这是一个用于验证代理配置修复的测试部门	{}	2025-08-10 12:35:36.932884+00	2025-08-11 11:51:45.515574+00	2025-08-10	2025-08-10	\N	f	f
1000035	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门九	DEPARTMENT	INACTIVE	1	/1000035	0		{}	2025-08-08 06:06:12.680371+00	2025-08-08 06:10:57.246611+00	2025-08-10	\N	\N	t	f
1000034	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试自动编码部门	DEPARTMENT	INACTIVE	1	/1000034	0		{}	2025-08-08 06:05:15.725628+00	2025-08-08 06:11:17.502145+00	2025-08-10	\N	\N	t	f
1000033	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	页面测试部门2	DEPARTMENT	INACTIVE	1	/1000033	0	测试页面编码自动生成功能	{}	2025-08-08 06:04:58.171327+00	2025-08-08 06:11:53.307689+00	2025-08-10	\N	\N	t	f
1000036	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	状态更新测试部门	DEPARTMENT	INACTIVE	1	/1000036	0		{}	2025-08-08 06:15:42.18495+00	2025-08-08 06:16:32.022373+00	2025-08-10	\N	\N	t	f
1000010	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	数据同步测试部门_FINAL	DEPARTMENT	ACTIVE	2	/1000000/1000010	0	用于验证数据同步的测试部门	{}	2025-08-06 09:11:59.778608+00	2025-08-09 05:41:18.00958+00	2025-08-10	\N	\N	t	f
1000005	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	运维部	DEPARTMENT	ACTIVE	2	/1000000/1000005	0	测试修复后的CDC实时同步	{}	2025-08-06 06:43:14.71988+00	2025-08-09 10:42:21.152661+00	2025-08-10	\N	\N	t	f
1000001	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	AI治理办公室	DEPARTMENT	ACTIVE	2	/1000000/1000001	0	技术研发部门	{"type": "rd"}	2025-08-05 11:23:01.426455+00	2025-08-09 12:07:15.838099+00	2025-08-10	\N	\N	t	f
1000006	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	同步测试部门禁用-公司-3级-描述4-排序7	COMPANY	INACTIVE	2	/1000000/1000006	7	描述4	{}	2025-08-06 07:00:33.177944+00	2025-08-10 03:55:14.63456+00	2025-08-10	\N	\N	t	f
1000081	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	代理修复测试部门_时态更新成功	DEPARTMENT	ACTIVE	1	/1000081	0	通过时态管理API进行的名称更新测试_最终验证	{}	2025-08-11 11:51:45.515574+00	2025-08-11 12:05:15.045274+00	2025-08-11	2025-08-11	测试时态记录创建修复_最终验证	f	f
1000040	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754668553755	DEPARTMENT	ACTIVE	1	/1000040	0	这是一个测试部门	{}	2025-08-08 15:55:53.759589+00	2025-08-08 15:55:53.760634+00	2025-08-10	\N	\N	t	f
1000041	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754669118696	DEPARTMENT	ACTIVE	1	/1000041	0	这是一个测试部门	{}	2025-08-08 16:05:18.711949+00	2025-08-08 16:05:18.712276+00	2025-08-10	\N	\N	t	f
1000042	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754669124970	DEPARTMENT	ACTIVE	1	/1000042	0	这是一个测试部门	{}	2025-08-08 16:05:24.97627+00	2025-08-08 16:05:24.976671+00	2025-08-10	\N	\N	t	f
1000043	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754669129397	DEPARTMENT	ACTIVE	1	/1000043	0	这是一个测试部门	{}	2025-08-08 16:05:29.412342+00	2025-08-08 16:05:29.412733+00	2025-08-10	\N	\N	t	f
1000044	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754669139984	DEPARTMENT	ACTIVE	1	/1000044	0	这是一个测试部门	{}	2025-08-08 16:05:40.001238+00	2025-08-08 16:05:40.001654+00	2025-08-10	\N	\N	t	f
1000045	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	全面测试部门	DEPARTMENT	ACTIVE	1	/1000045	0		{}	2025-08-08 23:33:02.396731+00	2025-08-08 23:33:02.398649+00	2025-08-10	\N	\N	t	f
1000046	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754703523513	DEPARTMENT	ACTIVE	1	/1000046	0	这是一个测试部门	{}	2025-08-09 01:38:43.274837+00	2025-08-09 01:38:43.276731+00	2025-08-10	\N	\N	t	f
1000047	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门_1754703527894	DEPARTMENT	ACTIVE	1	/1000047	0	这是一个测试部门	{}	2025-08-09 01:38:47.922057+00	2025-08-09 01:38:47.922372+00	2025-08-10	\N	\N	t	f
1000048	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十	DEPARTMENT	ACTIVE	1	/1000048	0		{}	2025-08-09 02:04:47.443405+00	2025-08-09 02:04:47.443898+00	2025-08-10	\N	\N	t	f
1000083	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态管理测试部门(升级版)	DEPARTMENT	ACTIVE	1	/1000083	0		{}	2025-08-10 12:58:56.152848+00	2025-08-10 12:59:54.466027+00	2025-08-20	2025-12-31	测试版本更新功能	t	t
1000049	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门0	DEPARTMENT	ACTIVE	1	/1000049	0	\N	{}	2025-08-09 03:01:03.157302+00	2025-08-09 03:01:03.158256+00	2025-08-10	\N	\N	t	f
1000050	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门1	DEPARTMENT	ACTIVE	1	/1000050	0	\N	{}	2025-08-09 03:01:03.324159+00	2025-08-09 03:01:03.324592+00	2025-08-10	\N	\N	t	f
1000051	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门2	DEPARTMENT	ACTIVE	1	/1000051	0	\N	{}	2025-08-09 03:01:03.451726+00	2025-08-09 03:01:03.452036+00	2025-08-10	\N	\N	t	f
1000052	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门3	DEPARTMENT	ACTIVE	1	/1000052	0	\N	{}	2025-08-09 03:01:03.576029+00	2025-08-09 03:01:03.576217+00	2025-08-10	\N	\N	t	f
1000089	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态测试组织-更新	DEPARTMENT	ACTIVE	1	/1000089	0	测试时态字段更新	{}	2025-08-10 14:26:47.809083+00	2025-08-10 14:29:52.26065+00	2024-06-01	2024-11-30	调整生效期间	t	f
1000081	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	代理修复测试部门_时态更新成功	DEPARTMENT	INACTIVE	1	/1000081	0	状态变更测试 - 设置为非活跃	{}	2025-08-11 12:05:15.045274+00	2025-08-11 12:06:09.568373+00	2025-08-12	2025-08-12	测试状态变更功能	f	f
1000054	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	Phase2测试部门	DEPARTMENT	ACTIVE	1	/1000054	0	验证简化测试	{}	2025-08-09 05:05:54.217338+00	2025-08-09 05:05:54.218356+00	2025-08-10	\N	\N	t	f
PERF001	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试组织_PERF001	DEPARTMENT	ACTIVE	1	/PERF001	1	单元测试组织	{}	2025-08-11 02:07:46.823179+00	2025-08-11 02:17:47.57412+00	2025-08-11	\N	\N	t	f
1000055	1000054	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	DDD简化更新测试	DEPARTMENT	INACTIVE	2	/1000054/1000055	0	测试层次计算	{}	2025-08-09 05:11:37.2748+00	2025-08-09 05:12:13.111427+00	2025-08-10	\N	\N	t	f
C722694	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC端到端测试	DEPARTMENT	ACTIVE	1	/C722694	1	验证CDC功能	{}	2025-08-09 06:58:14.346048+00	2025-08-09 06:58:14.346048+00	2025-08-10	\N	\N	t	f
S722950	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	数据同步测试	DEPARTMENT	ACTIVE	1	/S722950	1	验证Neo4j同步功能	{}	2025-08-09 07:02:30.110333+00	2025-08-09 07:02:30.110333+00	2025-08-10	\N	\N	t	f
U723048	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	解析修复测试	DEPARTMENT	ACTIVE	1	/U723048	1	验证unwrapped CDC事件	{}	2025-08-09 07:04:08.52621+00	2025-08-09 07:04:08.52621+00	2025-08-10	\N	\N	t	f
F723128	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC修复验证	DEPARTMENT	ACTIVE	1	/F723128	1	验证schema消息解析	{}	2025-08-09 07:05:28.554088+00	2025-08-09 07:05:28.554088+00	2025-08-10	\N	\N	t	f
1000053	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	稳定性测试部门4	DEPARTMENT	INACTIVE	1	/1000053	0	\N	{}	2025-08-09 03:01:03.703919+00	2025-08-09 08:16:23.560124+00	2025-08-10	\N	\N	t	f
1000058	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	同步测试部门_验证	DEPARTMENT	ACTIVE	1	/1000058	0		{}	2025-08-09 08:47:46.220656+00	2025-08-09 08:47:46.221894+00	2025-08-10	\N	\N	t	f
1000059	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	重新同步测试_部门	DEPARTMENT	ACTIVE	1	/1000059	0		{}	2025-08-09 08:49:46.600642+00	2025-08-09 08:49:46.602372+00	2025-08-10	\N	\N	t	f
1000057	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十一	DEPARTMENT	ACTIVE	1	/1000057	0		{}	2025-08-09 08:40:36.610696+00	2025-08-09 08:52:50.589984+00	2025-08-10	\N	\N	t	f
1000060	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	前端修复验证部门	DEPARTMENT	ACTIVE	1	/1000060	0	验证前端修复是否成功的测试部门	{}	2025-08-09 09:59:25.060107+00	2025-08-09 09:59:25.063766+00	2025-08-10	\N	\N	t	f
1000061	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	部门二十二	DEPARTMENT	INACTIVE	1	/1000061	0		{}	2025-08-09 10:07:00.322212+00	2025-08-09 10:07:21.47672+00	2025-08-10	\N	\N	t	f
1000062	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	E2E测试部门_缓存失效测试	DEPARTMENT	ACTIVE	1	/1000062	0		{}	2025-08-09 10:50:35.015368+00	2025-08-09 10:53:17.206794+00	2025-08-10	\N	\N	t	f
1000063	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试组织_1754736868	DEPARTMENT	ACTIVE	1	/1000063	0		{}	2025-08-09 10:54:28.700026+00	2025-08-09 10:54:28.70107+00	2025-08-10	\N	\N	t	f
1000064	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试组织(已更新)	DEPARTMENT	ACTIVE	2	/1000000/1000064	0		{}	2025-08-09 12:09:43.433069+00	2025-08-09 12:09:50.151959+00	2025-08-10	\N	\N	t	f
1000093	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	端到端测试组织	DEPARTMENT	ACTIVE	1	/1000093	0	测试创建的组织	{}	2025-08-11 09:35:47.987832+00	2025-08-11 09:35:47.989614+00	2025-08-11	\N	\N	t	f
1000065	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	端到端测试部门_已更新	DEPARTMENT	INACTIVE	2	/1000000/1000065	0		{}	2025-08-09 12:24:34.885664+00	2025-08-09 12:30:03.646997+00	2025-08-10	\N	\N	t	f
1000066	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	健康检查测试部门	DEPARTMENT	ACTIVE	1	/1000066	0		{}	2025-08-09 12:30:31.109551+00	2025-08-09 12:30:31.111004+00	2025-08-10	\N	\N	t	f
1000094	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	前端修复成功测试部门	DEPARTMENT	ACTIVE	1	/1000094	0		{}	2025-08-11 09:51:11.250898+00	2025-08-11 09:51:11.251966+00	2025-08-11	\N	\N	t	f
1000067	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	删除测试部门_1754742927	DEPARTMENT	INACTIVE	1	/1000067	0	用于测试删除功能	{}	2025-08-09 12:35:27.966243+00	2025-08-09 12:35:27.983511+00	2025-08-10	\N	\N	t	f
1000068	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC测试部门_1754742983	DEPARTMENT	ACTIVE	1	/1000068	0	CDC同步测试	{}	2025-08-09 12:36:28.515331+00	2025-08-09 12:36:28.516233+00	2025-08-10	\N	\N	t	f
1000069	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	缓存失效测试_1754743228	DEPARTMENT	ACTIVE	1	/1000069	0	测试缓存失效机制	{}	2025-08-09 12:40:35.286829+00	2025-08-09 12:40:35.288004+00	2025-08-10	\N	\N	t	f
1000070	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	缓存失效测试2_1754743246	DEPARTMENT	ACTIVE	1	/1000070	0	再次测试缓存失效	{}	2025-08-09 12:40:46.337728+00	2025-08-09 12:40:46.338665+00	2025-08-10	\N	\N	t	f
1000071	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_1	DEPARTMENT	ACTIVE	1	/1000071	0	性能压力测试	{}	2025-08-09 12:53:08.788862+00	2025-08-09 12:53:08.789868+00	2025-08-10	\N	\N	t	f
1000072	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_2	DEPARTMENT	ACTIVE	1	/1000072	0	性能压力测试	{}	2025-08-09 12:53:08.810222+00	2025-08-09 12:53:08.811267+00	2025-08-10	\N	\N	t	f
1000073	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_3	DEPARTMENT	ACTIVE	1	/1000073	0	性能压力测试	{}	2025-08-09 12:53:08.825371+00	2025-08-09 12:53:08.826261+00	2025-08-10	\N	\N	t	f
1000074	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_4	DEPARTMENT	ACTIVE	1	/1000074	0	性能压力测试	{}	2025-08-09 12:53:08.837663+00	2025-08-09 12:53:08.838478+00	2025-08-10	\N	\N	t	f
1000075	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试部门_1754743988_5	DEPARTMENT	ACTIVE	1	/1000075	0	性能压力测试	{}	2025-08-09 12:53:08.850776+00	2025-08-09 12:53:08.851811+00	2025-08-10	\N	\N	t	f
1000084	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态测试部门	DEPARTMENT	ACTIVE	1	/1000084	10	测试时态字段修复	{}	2025-08-10 13:35:20.789341+00	2025-08-10 13:35:20.7903+00	2024-08-10	2024-12-31	测试日期类型修复	t	t
1000085	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态测试部门2	DEPARTMENT	ACTIVE	1	/1000085	20	测试修复后的时态字段同步	{}	2025-08-10 13:42:51.280224+00	2025-08-10 13:42:51.281228+00	2024-08-11	2024-12-30	测试字段修复后的同步	t	t
1000086	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态同步测试3	DEPARTMENT	ACTIVE	1	/1000086	30	测试修复后的DebeziumDate处理	{}	2025-08-10 13:47:55.215923+00	2025-08-10 13:47:55.217765+00	2024-08-12	2024-12-29	测试DebeziumDate修复	t	t
1000076	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试编辑部门	COST_CENTER	ACTIVE	1	/1000076	0	完整编辑模式测试 - 所有字段可编辑	{}	2025-08-10 03:02:51.529857+00	2025-08-10 03:15:03.677593+00	2025-08-10	\N	\N	t	f
1000087	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	最终测试部门	DEPARTMENT	ACTIVE	1	/1000087	40	最终验证修复	{}	2025-08-10 13:51:06.866115+00	2025-08-10 13:51:06.867187+00	2024-08-13	2024-12-28	最终DebeziumDate测试	t	t
1000088	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	调试测试部门	DEPARTMENT	ACTIVE	1	/1000088	50	调试RawMessage处理	{}	2025-08-10 13:53:03.469378+00	2025-08-10 13:53:03.470626+00	2024-08-14	2024-12-27	RawMessage调试测试	t	t
1000013	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	前端修复测试组织	DEPARTMENT	INACTIVE	1	/1000013	0	测试前端修复是否生效	{}	2025-08-07 12:59:43.272937+00	2025-08-11 12:07:02.277179+00	2025-08-10	2025-08-16	\N	f	f
1000081	1000082	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	重组后的测试部门	DEPARTMENT	INACTIVE	2	/1000082/1000081	0	状态变更测试 - 设置为非活跃	{}	2025-08-11 12:06:09.568373+00	2025-08-11 12:10:15.569162+00	2025-08-13	2025-08-19	组织架构重组 - 调整上级关系	f	f
1000081	1000082	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	性能测试1	DEPARTMENT	INACTIVE	2	/1000082/1000081	0	状态变更测试 - 设置为非活跃	{}	2025-08-11 12:10:15.569162+00	2025-08-11 12:10:15.569162+00	2025-08-20	\N	性能测试	t	f
TEST001	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试组织_TEST001	DEPARTMENT	ACTIVE	1	/TEST001	1	测试更新描述	{}	2025-08-11 02:07:46.775966+00	2025-08-11 02:17:47.552489+00	2025-08-12	\N	单元测试	t	f
1000091	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	MCP浏览器测试部门	DEPARTMENT	ACTIVE	1	/1000091	0	用于测试MCP浏览器功能的部门	{}	2025-08-11 05:16:50.076445+00	2025-08-11 05:16:50.077533+00	2025-08-11	\N		t	f
1000009	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	GraphQL端到端测试部门_修复验证	DEPARTMENT	ACTIVE	2	/1000000/1000009	0	用于验证GraphQL集成的测试部门	{}	2025-08-06 08:53:30.92514+00	2025-08-11 05:34:30.736682+00	2025-08-10	\N	\N	t	f
1000077	1000000	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	生产环境测试部门	DEPARTMENT	ACTIVE	2	/1000000/1000077	0	生产环境部署验证测试	{}	2025-08-10 06:27:01.951103+00	2025-08-10 06:27:01.952358+00	2025-08-10	\N	\N	t	f
1000080	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	CDC同步测试部门	DEPARTMENT	ACTIVE	1	/1000080	0	测试CDC数据同步功能	{}	2025-08-10 12:02:56.262746+00	2025-08-10 12:02:56.262746+00	2025-08-10	\N	\N	t	f
1000092	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态管理测试组织v2-重组测试	DEPARTMENT	ACTIVE	1	/1000092	0	前端编辑表单功能测试版本1	{}	2025-08-11 07:49:33.249268+00	2025-08-11 07:49:45.709675+00	2025-09-01	\N	组织重组升级	t	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试部门 - 时态管理功能验证v7	DEPARTMENT	ACTIVE	1	/1000056	0	用于验证时态管理API的完整功能	{}	2025-08-11 15:27:51.581762+00	2025-08-11 16:02:56.788915+00	2025-08-12	2025-08-31	全面测试时态管理功能	f	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	时态管理测试部门v10_新增功能验证	DEPARTMENT	ACTIVE	1	/1000056	0	部门重组优化	{}	2025-08-11 17:00:58.236942+00	2025-08-11 17:05:45.532482+00	2026-01-01	2026-02-11	测试新增版本功能 - 验证时态管理系统的版本创建能力	f	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	重组测试部门v3	DEPARTMENT	ACTIVE	1	/1000056	0	部门重组优化	{}	2024-01-01 00:00:00+00	2025-08-11 16:02:56.788915+00	2024-01-01	2024-12-31	年度重组计划	f	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	初创测试部门v1	DEPARTMENT	ACTIVE	1	/1000056	0	新成立的测试部门	{}	2023-01-01 00:00:00+00	2025-08-11 16:02:56.788915+00	2023-01-01	2023-12-31	部门成立	f	f
1000056	1000001	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	重组后的时态管理测试部门v8	COST_CENTER	ACTIVE	3	/1000000/1000001/1000056	0	组织架构重组后的新定位	{}	2025-08-11 15:28:57.330404+00	2025-08-11 16:02:56.788915+00	2025-09-01	2025-11-30	季度组织架构重组	f	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	全新重组测试部门v9	COST_CENTER	ACTIVE	3	/1000000/1000001/1000056	0	组织架构重组后的新定位	{}	2025-08-11 15:55:01.549001+00	2025-08-11 16:02:56.788915+00	2025-12-01	\N	年终组织架构重组测试	f	f
1001000	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	健康检查测试部门	DEPARTMENT	ACTIVE	1	/1001000	0		{}	2025-08-11 16:54:49.225065+00	2025-08-11 16:54:49.226238+00	2025-08-12	\N	\N	t	f
1001001	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	健康检查测试部门	DEPARTMENT	ACTIVE	1	/1001001	0		{}	2025-08-11 16:56:34.006871+00	2025-08-11 16:56:34.007966+00	2025-08-12	\N	\N	t	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	过渡期测试部门v5	DEPARTMENT	ACTIVE	1	/1000056	0	部门重组优化	{}	2025-08-11 15:53:26.28019+00	2025-08-11 17:00:58.236942+00	2025-01-01	2025-12-31	时态连续性修复 - 填补缺失时间段	f	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	手工测试	DEPARTMENT	ACTIVE	1	/1000056	0	部门重组优化	{}	2025-08-11 17:05:45.532482+00	2025-08-11 17:19:05.972483+00	2026-02-12	2026-02-28	测试目的	f	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	优化测试部门 - 集成表单版	DEPARTMENT	ACTIVE	1	/1000056	0	部门重组优化	{}	2025-08-11 17:19:05.972483+00	2025-08-11 17:23:55.49076+00	2026-03-01	2027-08-11	测试新增版本功能优化 - 验证表单集成到右侧详情区的用户体验改进效果	f	f
1000056	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	测试增加时态版本	DEPARTMENT	ACTIVE	1	/1000056	0	部门重组优化	{}	2025-08-11 17:23:55.49076+00	2025-08-11 17:23:55.49076+00	2027-08-12	\N	测算输出	t	f
1001002	\N	3b99930c-4dc6-4cc9-8e4d-7d960a931cb9	健康检查测试部门	DEPARTMENT	ACTIVE	1	/1001002	0		{}	2025-08-12 04:23:51.705244+00	2025-08-12 04:23:51.70623+00	2025-08-12	\N	\N	t	f
\.
...
--
-- Data for Name: temporal_gap_audit; Type: TABLE DATA; Schema: public; Owner: user
--

COPY public.temporal_gap_audit (id, organization_code, gap_type, deactivated_period, fill_strategy, fill_success, audit_message, created_at) FROM stdin;
3	1000003	MIDDLE_GAP	2011-01-01 to 2015-12-31	EXTEND_PREVIOUS	t	前向扩展成功: 销售部门 (2006-2010) 扩展至 2015-12-31	2025-08-13 08:22:42.063579
4	1000003	TRIGGER_EXECUTION	UPDATE [2011-01-01 to 2015-12-31]	AUTO_TRIGGER	t	触发器执行 UPDATE 操作，自动填充结果: 成功	2025-08-13 08:22:42.063579
26	PERF001	ISOLATED_GAP	2021-01-01 to 2021-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
27	PERF001	TRIGGER_EXECUTION	DELETE [2021-01-01 to 2021-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
28	PERF002	ISOLATED_GAP	2022-01-01 to 2022-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
29	PERF002	TRIGGER_EXECUTION	DELETE [2022-01-01 to 2022-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
30	PERF003	ISOLATED_GAP	2023-01-01 to 2023-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
31	PERF003	TRIGGER_EXECUTION	DELETE [2023-01-01 to 2023-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
32	PERF004	ISOLATED_GAP	2024-01-01 to 2024-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
33	PERF004	TRIGGER_EXECUTION	DELETE [2024-01-01 to 2024-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
34	PERF005	ISOLATED_GAP	2025-01-01 to 2025-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
35	PERF005	TRIGGER_EXECUTION	DELETE [2025-01-01 to 2025-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
36	PERF006	ISOLATED_GAP	2026-01-01 to 2026-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
37	PERF006	TRIGGER_EXECUTION	DELETE [2026-01-01 to 2026-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
38	PERF007	ISOLATED_GAP	2027-01-01 to 2027-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
39	PERF007	TRIGGER_EXECUTION	DELETE [2027-01-01 to 2027-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
40	PERF008	ISOLATED_GAP	2028-01-01 to 2028-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
41	PERF008	TRIGGER_EXECUTION	DELETE [2028-01-01 to 2028-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
42	PERF009	ISOLATED_GAP	2029-01-01 to 2029-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
43	PERF009	TRIGGER_EXECUTION	DELETE [2029-01-01 to 2029-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
44	PERF010	ISOLATED_GAP	2030-01-01 to 2030-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:36:26.429902
45	PERF010	TRIGGER_EXECUTION	DELETE [2030-01-01 to 2030-12-31]	AUTO_TRIGGER	f	触发器执行 DELETE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:36:26.429902
46	PERF001	MIDDLE_GAP	2021-01-01 to 2021-12-31	EXTEND_PREVIOUS	t	前向扩展成功: 性能测试A 从 2020-12-31 扩展至 2021-12-31	2025-08-13 08:36:26.429902
47	PERF001	TRIGGER_EXECUTION	UPDATE [2021-01-01 to 2021-12-31]	AUTO_TRIGGER	t	触发器执行 UPDATE 操作，自动填充结果: 成功	2025-08-13 08:36:26.429902
48	1000003	MIDDLE_GAP	2021-01-01 to 2024-12-31	EXTEND_PREVIOUS	t	前向扩展成功: 销售中心 从 2020-12-31 扩展至 2024-12-31	2025-08-13 08:49:54.270403
49	1000003	TRIGGER_EXECUTION	DELETE [2021-01-01 to 2024-12-31]	AUTO_TRIGGER	t	触发器执行 DELETE 操作，自动填充结果: 成功	2025-08-13 08:49:54.270403
50	1000003	ISOLATED_GAP	2026-01-01 to 2026-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:53:01.127391
51	1000003	TRIGGER_EXECUTION	UPDATE [2026-01-01 to 2026-12-31]	AUTO_TRIGGER	f	触发器执行 UPDATE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:53:01.127391
52	1000003	ISOLATED_GAP	2027-01-01 to 	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:53:01.127391
53	1000003	TRIGGER_EXECUTION	UPDATE [2027-01-01 to ]	AUTO_TRIGGER	f	触发器执行 UPDATE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:53:01.127391
54	1000003	ISOLATED_GAP	2016-01-01 to 2024-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:53:01.127391
55	1000003	TRIGGER_EXECUTION	UPDATE [2016-01-01 to 2024-12-31]	AUTO_TRIGGER	f	触发器执行 UPDATE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:53:01.127391
56	1000003	ISOLATED_GAP	2000-01-01 to 2000-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:53:01.127391
57	1000003	TRIGGER_EXECUTION	UPDATE [2000-01-01 to 2000-12-31]	AUTO_TRIGGER	f	触发器执行 UPDATE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:53:01.127391
58	1000003	ISOLATED_GAP	2001-01-01 to 2005-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:53:01.127391
59	1000003	TRIGGER_EXECUTION	UPDATE [2001-01-01 to 2005-12-31]	AUTO_TRIGGER	f	触发器执行 UPDATE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:53:01.127391
60	1000003	ISOLATED_GAP	2006-01-01 to 2015-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:53:01.127391
61	1000003	TRIGGER_EXECUTION	UPDATE [2006-01-01 to 2015-12-31]	AUTO_TRIGGER	f	触发器执行 UPDATE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:53:01.127391
62	1000003	ISOLATED_GAP	2025-01-01 to 2025-12-31	MANUAL_REQUIRED	f	无相邻记录可扩展，需要手动处理	2025-08-13 08:53:01.127391
63	1000003	TRIGGER_EXECUTION	UPDATE [2025-01-01 to 2025-12-31]	AUTO_TRIGGER	f	触发器执行 UPDATE 操作，自动填充结果: 失败或需要手动处理	2025-08-13 08:53:01.127391
64	1000003	MIDDLE_GAP	2025-01-01 to 2025-12-31	EXTEND_PREVIOUS	t	前向扩展成功: 销售中心 从 2024-12-31 扩展至 2025-12-31	2025-08-13 09:02:41.282216
65	1000003	TRIGGER_EXECUTION	UPDATE [2025-01-01 to 2025-12-31]	AUTO_TRIGGER	t	触发器执行 UPDATE 操作，自动填充结果: 成功	2025-08-13 09:02:41.282216
66	1000003	MIDDLE_GAP	2016-01-01 to 2025-12-31	EXTEND_PREVIOUS	t	前向扩展成功: 销售部门 从 2015-12-31 扩展至 2025-12-31	2025-08-13 09:28:43.218135
67	1000003	TRIGGER_EXECUTION	UPDATE [2016-01-01 to 2025-12-31]	AUTO_TRIGGER	t	触发器执行 UPDATE 操作，自动填充结果: 成功	2025-08-13 09:28:43.218135
68	1000003	ERROR	\N	\N	f	全时间轴重建失败: value too long for type character varying(20)	2025-08-13 10:30:11.307619
69	1000003	FULL_TIMELINE_REBUILD	\N	INTELLIGENT_EXTENSION	t	全时间轴重建完成，执行0次操作：	2025-08-13 10:30:44.549892
70	1000003	SMART_TIMELINE_FILL	\N	EXTEND_TO_NEXT	t	智能填充完成，执行1次操作：填充空洞：扩展 销售事业部 从 2015-12-31 到 2025-12-31 00:00:00; 	2025-08-13 10:32:32.713272
71	1000003	OVERLAP_CORRECTION	\N	ADJUST_END_DATE	t	修复重叠问题：调整销售部门结束日期从2025-12-31到2010-12-31，避免与销售事业部重叠	2025-08-13 10:33:53.053866
72	1000003	TIMELINE_VALIDATION	\N	FINAL_VERIFICATION	t	✅ 时间轴优化完成：ACTIVE记录完全连续，无空隙无重叠，从2000-01-01到未来	2025-08-13 10:34:07.962388
73	1000003	MIDDLE_GAP	2006-01-01 to 2010-12-31	EXTEND_PREVIOUS	t	前向扩展成功: 销售团队 从 2005-12-31 扩展至 2010-12-31	2025-08-13 10:47:57.414016
74	1000003	TRIGGER_EXECUTION	UPDATE [2006-01-01 to 2010-12-31]	AUTO_TRIGGER	t	触发器执行 UPDATE 操作，自动填充结果: 成功	2025-08-13 10:47:57.414016
75	1000003	SMART_TIMELINE_FILL	\N	EXTEND_TO_NEXT	t	时间轴已连贯，无需填充	2025-08-13 10:47:57.414016
\.
...
SELECT pg_catalog.setval('public.org_unit_code_seq', 1000082, true);
...
--
-- Name: temporal_gap_audit_id_seq; Type: SEQUENCE SET; Schema: public; Owner: user
--

SELECT pg_catalog.setval('public.temporal_gap_audit_id_seq', 75, true);
...
--
-- Name: organization_events organization_events_pkey; Type: CONSTRAINT; Schema: public; Owner: user
--

ALTER TABLE ONLY public.organization_events
    ADD CONSTRAINT organization_events_pkey PRIMARY KEY (event_id);
...
ALTER TABLE ONLY public.organization_units
    ADD CONSTRAINT organization_units_pkey PRIMARY KEY (code, effective_date);
...
--
-- Name: temporal_gap_audit temporal_gap_audit_pkey; Type: CONSTRAINT; Schema: public; Owner: user
--

ALTER TABLE ONLY public.temporal_gap_audit
    ADD CONSTRAINT temporal_gap_audit_pkey PRIMARY KEY (id);
...
--
-- Name: idx_current_organizations_list; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_current_organizations_list ON public.organization_units USING btree (tenant_id, is_current, status, effective_date DESC) WHERE (is_current = true);


--
-- Name: idx_current_record_fast; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_current_record_fast ON public.organization_units USING btree (tenant_id, code, is_current) WHERE (is_current = true);
...
--
-- Name: idx_org_current_active; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_current_active ON public.organization_units USING btree (tenant_id, code, is_current) WHERE (is_current = true);


--
-- Name: idx_org_current_version; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_current_version ON public.organization_units USING btree (code, is_current) WHERE (is_current = true);


--
-- Name: idx_org_date_range; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_date_range ON public.organization_units USING btree (tenant_id, effective_date DESC, end_date DESC NULLS LAST) WHERE (effective_date IS NOT NULL);


--
-- Name: idx_org_effective_date; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_effective_date ON public.organization_units USING btree (effective_date);


--
-- Name: idx_org_events_code; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_events_code ON public.organization_events USING btree (organization_code);


--
-- Name: idx_org_events_data_gin; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_events_data_gin ON public.organization_events USING gin (event_data);


--
-- Name: idx_org_events_date; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_events_date ON public.organization_events USING btree (effective_date);


--
-- Name: idx_org_events_lookup; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_events_lookup ON public.organization_events USING btree (tenant_id, organization_code, effective_date DESC, event_type);


--
-- Name: idx_org_events_tenant; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_events_tenant ON public.organization_events USING btree (tenant_id);


--
-- Name: idx_org_events_timeline; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_events_timeline ON public.organization_events USING btree (tenant_id, effective_date DESC, created_at DESC);


--
-- Name: idx_org_events_type; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_events_type ON public.organization_events USING btree (event_type);


--
-- Name: idx_org_history_lookup; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_history_lookup ON public.organization_units USING btree (tenant_id, code, created_at DESC, effective_date DESC) WHERE (is_current = false);


--
-- Name: idx_org_status_type; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_status_type ON public.organization_units USING btree (tenant_id, status, unit_type, effective_date DESC);
...
--
-- Name: idx_org_temporal_core; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_temporal_core ON public.organization_units USING btree (tenant_id, code, effective_date DESC, end_date DESC NULLS LAST) WHERE (effective_date IS NOT NULL);
...
--
-- Name: idx_org_temporal_query; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_temporal_query ON public.organization_units USING btree (tenant_id, code, effective_date, end_date);


--
-- Name: idx_org_temporal_range_composite; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_org_temporal_range_composite ON public.organization_units USING btree (tenant_id, code, effective_date DESC, end_date DESC NULLS LAST, is_current, status) WHERE (effective_date IS NOT NULL);
...
--
-- Name: idx_organization_current_only; Type: INDEX; Schema: public; Owner: user
--
...
--
-- Name: idx_organization_date_range; Type: INDEX; Schema: public; Owner: user
--
...
--
-- Name: idx_organization_events_optimized; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_organization_events_optimized ON public.organization_events USING btree (organization_code, effective_date DESC, created_at DESC);


--
-- Name: idx_organization_temporal_main; Type: INDEX; Schema: public; Owner: user
--
...
--
-- Name: idx_temporal_gap_audit_created_at; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_temporal_gap_audit_created_at ON public.temporal_gap_audit USING btree (created_at);


--
-- Name: idx_temporal_gap_audit_org_code; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_temporal_gap_audit_org_code ON public.temporal_gap_audit USING btree (organization_code);


--
-- Name: idx_temporal_range_optimized; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_temporal_range_optimized ON public.organization_units USING btree (tenant_id, code, effective_date, end_date) WHERE (effective_date IS NOT NULL);


--
-- Name: idx_temporal_range_query; Type: INDEX; Schema: public; Owner: user
--

CREATE INDEX idx_temporal_range_query ON public.organization_units USING btree (code, effective_date, end_date) WHERE (effective_date IS NOT NULL);
...
--
-- Name: uk_current_organization; Type: INDEX; Schema: public; Owner: user
--

CREATE UNIQUE INDEX uk_current_organization ON public.organization_units USING btree (code) WHERE (is_current = true);
...
--
-- Name: organization_units temporal_gap_auto_fill; Type: TRIGGER; Schema: public; Owner: user
--

CREATE TRIGGER temporal_gap_auto_fill AFTER DELETE OR UPDATE ON public.organization_units FOR EACH ROW EXECUTE FUNCTION public.temporal_gap_auto_fill_trigger();
...
--
-- Name: TABLE organization_current; Type: ACL; Schema: public; Owner: user
--

GRANT SELECT ON TABLE public.organization_current TO debezium_user;


--
-- Name: TABLE organization_events; Type: ACL; Schema: public; Owner: user
--

GRANT SELECT ON TABLE public.organization_events TO debezium_user;
...
--
-- Name: TABLE organization_units_backup_pre_temporal; Type: ACL; Schema: public; Owner: user
--

GRANT SELECT ON TABLE public.organization_units_backup_pre_temporal TO debezium_user;


--
-- Name: TABLE organization_units_backup_pre_version_removal; Type: ACL; Schema: public; Owner: user
--

GRANT SELECT ON TABLE public.organization_units_backup_pre_version_removal TO debezium_user;


--
-- Name: TABLE organization_units_backup_temporal; Type: ACL; Schema: public; Owner: user
--

GRANT SELECT ON TABLE public.organization_units_backup_temporal TO debezium_user;
...
--
-- Name: TABLE v_temporal_query_performance; Type: ACL; Schema: public; Owner: user
--

GRANT SELECT ON TABLE public.v_temporal_query_performance TO debezium_user;
