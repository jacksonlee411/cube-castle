...
### 9. 开发前强制检查原则 (Pre-Development Mandatory Check Principle) ⭐ **重大修订 (2025-08-26)**

#### 9.1 API契约优先检查 🚨 **开发绝对起点**
- **两份核心文件**: 开发任何功能前必须先检查以下两份API契约文件：
  - `/home/shangmeilin/cube-castle/docs/api/openapi.yaml` - REST命令操作权威规范
  - `/home/shangmeilin/cube-castle/docs/api/schema.graphql` - GraphQL查询操作权威Schema
- **强制顺序**: API契约检查 → 功能存在性检查 → 开发实现
- **契约驱动**: 所有开发必须严格基于契约文件，不得脱离契约自行设计API

#### 9.2 现有资源深度分析 🔍 **优先使用原则** ⭐ **重大强化 (2025-09-09)**
- **分析优先**: 开发任何功能前，必须深度分析提供的上下文文件和现有代码库
- **优先使用**: 如果存在可用的API、接口、函数、组件，必须直接使用，禁止重复创建
- **创建条件**: 只有在确认不存在可用资源后，才能提出创建新函数/组件的建议
- **强制检查范围**: 
  - **API端点存在性**: 验证现有REST/GraphQL端点是否已提供所需功能
  - **函数/方法搜索**: 使用Grep工具搜索相关函数、类、接口
  - **组件复用检查**: 前端组件、Hook、工具函数的可复用性分析
  - **类型定义查找**: TypeScript接口、Go结构体的现有定义
  - **配置文件分析**: 检查现有配置、常量、枚举定义
  - **工具函数评估**: 分析shared/utils目录下的可用工具函数
- **上下文文件优先**: 优先分析用户提供的上下文文件中的可用资源

#### 9.3 强制检查流程 📋 **严格执行顺序**
```yaml
开发前必检流程 (按顺序执行):
  第一步: 检查API契约文件
    - 查阅 openapi.yaml 确认REST端点规范
    - 查阅 schema.graphql 确认GraphQL查询Schema
    - 理解现有API设计和数据模型
  
  第二步: 现有资源深度分析 ⭐ 新增强化
    - 分析上下文文件中的可用API/函数/组件
    - 搜索相关功能关键词和现有实现
    - 检查shared目录下的工具函数和类型定义
    - 验证现有Hook、组件、工具函数的可复用性
    - 评估现有配置文件和常量定义
  
  第二步补充: 运行实现清单生成器 🚨 **强制执行 (2025-09-09)**
    - 执行: node scripts/generate-implementation-inventory.js
    - 分析输出的现有实现清单
    - 验证新功能是否与现有组件重复
    - 确认是否可复用已有的处理器、服务或前端组件
  ...
  第五步: 新功能强制登记 🚨 **避免重复造轮子 (2025-09-09)**
    - 新增任何API端点、处理器、服务、组件后
    - 必须重新运行 node scripts/generate-implementation-inventory.js
    - 验证新功能已正确出现在实现清单中
    - 更新相关文档，确保团队成员知晓新增资源
```

#### 9.4 严格禁止事项 ❌ **零容忍**
- **API契约违规**: 不检查API契约文件就开始开发
- **重复创建资源**: 在存在可用API/函数/组件时重复创建相同功能
- **忽视现有实现**: 不分析上下文文件和现有代码就开始新功能开发
- **脱离契约**: 忽视API规范文件自行设计接口
- **忽视已有成果**: 忽视已有的工作成果和架构设计
- **跳过资源分析**: 直接提出创建新函数而不先分析现有可用资源
- **无效重复劳动**: 表现得像"白痴"一样忘记检查基础文档和功能
- **跳过实现清单检查**: 不运行 generate-implementation-inventory.js 就开始开发新功能 🚨 **新增 (2025-09-09)**
- **新功能未登记**: 新增功能后不更新实现清单和文档，导致重复造轮子 🚨 **新增 (2025-09-09)**
...
### 16. API端口配置绝对禁止原则 (API Port Configuration Absolute Prohibition) ⭐ **严重警告 (2025-08-31)**

#### 16.1 端口配置神圣不可侵犯 🚨 **零容忍政策**
- **绝对禁止**: 任何人不得随意修改API端口配置，违者严重后果自负
- **标准端口配置**: 
  - 后端服务器: 9090端口 (所有REST API命令操作)
  - GraphQL查询服务: 8090端口 (所有查询操作)
  - 前端开发服务器: 3000/3001端口 (开发环境)
- **Vite代理配置权威**: `/home/shangmeilin/cube-castle/frontend/vite.config.ts` 中的端口配置为绝对权威
- **历史惨痛教训**: 2025-08-31修复了因错误配置9091端口导致的前端删除功能500错误

#### 16.2 端口配置变更严格管控 🔒 **强制执行**
- **变更禁令**: 严格禁止未经用户明确授权修改任何API端点的目标端口
- **一致性要求**: 所有相关文件中的端口配置必须保持绝对一致
- **影响文件清单**:
  ```yaml
  关键配置文件:
    - vite.config.ts: Vite开发服务器代理配置 (第22-31行)
    - 测试文件: 所有E2E测试中的API_URL配置
    - 文档文件: README、CLAUDE.md中的端口说明
    - 脚本文件: 启动脚本中的端口引用
  ```

#### 16.3 端口配置错误的严重后果 ⚠️ **历史教训**
- **前端功能完全失效**: API请求发送到错误端口导致ECONNREFUSED错误
- **用户体验严重受损**: 500错误误导用户以为是服务器问题
- **调试时间巨大浪费**: 需要花费大量时间定位配置错误
- **系统稳定性威胁**: 错误的端口配置可能导致系统间通信中断

#### 16.4 端口配置验证机制 📋 **强制检查**
- **开发前检查**: 每次开发前必须验证所有端口配置的一致性
- **代码审查要点**: 任何涉及端口配置的变更必须经过严格审查
- **自动化验证**: CI/CD管道应包含端口配置一致性检查
- **文档同步**: 端口配置变更必须同时更新所有相关文档

#### 16.5 违反端口配置原则的严厉处罚 ❌ **零容忍**
- **立即回滚**: 发现端口配置错误必须立即回滚所有变更
- **根本原因分析**: 必须进行深入的根本原因分析和教训总结
- **预防措施**: 建立更严格的端口配置管理和验证机制
- **严重警告**: 再次发生端口配置错误将面临严厉后果

### 17. 深层次API一致性原则 (Deep API Consistency Principles) ⭐ **重大新增 (2025-08-23)**

#### 17.1 响应结构一致性 🚨 **企业级信封标准**
...
#### **案例3: 数据库触发器深度优化 (2025-09-07)** ⭐ **S级成功完成**
**性能优化与技术债务清理**: 解决12个触发器复杂性问题，消除功能冲突和重复执行
- **问题识别**: 12个触发器存在严重功能重叠，auto_end_date_trigger与simple_temporal_gap_fill_trigger冲突
- **精准诊断**: 直接查询PostgreSQL系统表获得准确状态，发现文档低估了问题严重性  
- **优化策略**: 激进式禁用7个问题触发器，保留5个核心功能触发器
- **实施成果**: 触发器数量减少58% (12→5)，0个业务功能影响，时态监控服务正常运行
- **性能提升**: 写操作响应时间稳定在12-20ms，系统资源使用优化，监控指标收集2-4ms
- **风险控制**: 完整备份和回滚脚本就绪，SQL语法错误修复(`overlaps`→`timeline_overlaps`)
- **成功关键**: 基于实际数据库状态的精准分析，而非依赖文档描述的保守估计

## 项目概述
Cube Castle是一个基于CQRS架构的组织架构管理系统，包含前端React应用和Go后端API服务。项目已完成PostgreSQL原生架构革命和契约测试自动化验证体系建设，实现了极致性能优化和企业级质量保证。

## 🏆 **项目当前状态 - 企业级生产就绪** ⭐ **重大更新 (2025-09-07)**
...
### 🏗️ PostgreSQL单表时态架构 (架构分析中)
- **架构模式**: 单表多版本时态架构设计已识别
- **主键设计**: 复合主键 `(code, effective_date)` 设计存在，多版本支持待验证
- **时态索引**: 26个专用时态索引存在，实际查询性能待测试
- **协议分离**: GraphQL和REST端点存在，CQRS实现程度需评估
- **数据验证**: 发现1个多版本数据示例，整体时态功能稳定性待验证
...
### 🧪 功能验证 (持续进行中)
- **时态架构理解**: 单表多版本架构设计已识别，存在少量历史版本数据
- **多版本管理**: 复合主键 `(code, effective_date)` 设计支持版本演进
- **时态查询能力**: 26个专用索引存在，性能表现需进一步验证
- **状态管理机制**: `is_current/is_future` 版本状态字段可用
...
### 🔧 架构理解进展
1. **时态架构识别**: 单表多版本时态架构设计已识别，实际运行稳定性需持续观察
2. **数据库设计**: PostgreSQL复合主键 `(code, effective_date)` 设计存在，查询性能待实际测试
3. **版本管理**: `is_current/is_future` 状态字段存在，自动化触发器逻辑需进一步验证
4. **事务一致性**: 单表架构理论支持事务原子性，实际并发表现待压力测试
5. **架构选择**: PostgreSQL单一数据源方案，复杂性降低但功能覆盖需验证

### 🔄 当前开发重点
1. **Canvas Kit v13兼容性**: 🔄 前端组件库升级进行中
2. **TypeScript类型统一**: 🔄 时态类型系统与前端集成优化中  
3. **API规范统一**: 🔄 OpenAPI契约与GraphQL Schema一致性完善中
4. **前后端集成**: 🔄 时态查询API与前端组件集成验证中
...
### 核心规范
- **现有资源优先**: 开发前必须深度分析上下文文件，优先使用现有API/函数/组件 ⭐ **新增强化**
...
### 禁止事项
- **重复创建资源**: 在存在可用API/函数/组件时重复创建相同功能 ⭐ **新增强化**
- **忽视现有实现**: 不分析上下文文件就直接提出创建新函数/组件
- **架构回退**: 不得重新引入Neo4j或双数据库架构
- **性能妥协**: 不得为了开发便利牺牲PostgreSQL原生性能优势
- **临时方案**: 不得使用未记录的权宜之计
- **重复开发**: 不得在未检查现有功能的情况下重复实现
- **数据同步**: 禁止重新引入CDC同步或双数据源架构
- **1000000组织数据保护**: 严格禁止任何agent修改、删除或更改组织编码为1000000的组织信息，该组织作为系统核心测试数据受到特殊保护
...
🔍 P3.1 自动化重复检测系统:
  ...
  - 重复代码率: 2.11% (远低于5%阈值)
  - 检测覆盖: 91个前端文件
  - CI/CD集成: duplicate-code-detection.yml工作流
...
🏗️ P3.2 架构守护规则系统:
  ...
  - 违规检测: 25个精确识别
  - Pre-commit集成: 自动架构一致性验证
  - 本地工具: scripts/quality/architecture-validator.js
...
📝 P3.3 文档自动同步系统:
  ...
  - 冲突检测: 8个不一致问题识别
  - 定时监控: 每日09:00自动检查
  - 本地工具: scripts/quality/document-sync.js
```

### 🎯 防控系统覆盖能力
- **重复代码防护**: 阻止>5%重复率代码合并
- **架构一致性**: 强制CQRS+端口+契约标准
- **文档同步**: 确保API规范与实现文档一致
- **质量门禁**: 三层纵深防御（本地+CI/CD+部署）

### 📊 防控效果指标
- **代码重复度**: 从85%+降至2.11% (96%+消除)
- **架构违规率**: 25个已识别，100%可追踪
- **文档同步率**: 80%目标，当前20%需改善
- **自动化程度**: 100%防控流程自动化

## 联系与维护
- 项目路径: `/home/shangmeilin/cube-castle`
- 最后更新: 2025-09-09
- 当前版本: **企业级生产就绪版 (v4.3-P3-Defense-System)** ⭐ **重大版本升级**
  - ⭐ **parentCode字段变更为必填**: 所有组织必须维护上级组织编码，根组织使用"0"
  - 🗄️ **数据库完成清理**: 删除测试数据，保留1000000组织作为根组织
  - 📋 **API规范统一更新**: OpenAPI和GraphQL Schema同步更新parentCode为必填字段
  - 🔧 **架构一致性增强**: 确保组织层级关系的完整性和数据一致性
  - 📖 **文档全面更新**: API文档新增parentCode必填要求说明和迁移指南
